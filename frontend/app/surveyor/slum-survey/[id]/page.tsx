...
export default function SlumSurveyPage() {
  ...
  const unwrappedSearchParams = React.useMemo(
    () => searchParams,
    [searchParams],
  );
  ...
  useEffect(() => {
    if (slum && slum.slumName) {
      ...
      setFormData((prev) => ({
        ...
        noSlumHouseholds:
          slum.totalHouseholds !== undefined
            ? slum.totalHouseholds
            : prev.noSlumHouseholds,
  ...
  const steps = [
    ...
    {
      title: isEditMode ? "Edit & Submit" : "Review & Submit",
      id: "reviewAndSubmit",
    },
  ];
      ...
      try {
        ...
        if (assignmentResponse.success && assignmentResponse.data) {
          ...
          if (
            ...
            (assignmentResponse.data as any)?.slum?.ward
          ) {
          ...
          if (slumResponse.success) {
            ...
            setFormData((prev) => ({
              ...
              noSlumHouseholds:
                slumData.totalHouseholds !== undefined
                  ? slumData.totalHouseholds
                  : 0,

              // Populate ward information from assignment if available
              wardNumber:
                assignmentWardData?.number ||
                assignmentWardData?.wardNumber ||
                "",
            ...
            if (slumData.stateCode) {
              try {
                ...
                if (statesResponse.success) {
                  const state = (statesResponse.data as any[]).find(
                    (s: any) => s.code === slumData.stateCode,
                  );
                  if (state) {
                    setFormData((prev) => ({
                      ...prev,
            ...
            if (slumData.distCode) {
              try {
                ...
                if (districtsResponse.success) {
                  const district = (districtsResponse.data as any[]).find(
                    (d: any) => d.code === slumData.distCode,
                  );
                  if (district) {
                    setFormData((prev) => ({
                      ...prev,
            ...
            if (
              ...
              !(
                assignmentWardData &&
                (assignmentWardData.number || assignmentWardData.name)
              )
            ) {
              try {
                const wardResponse = await apiService.get(
                  `/admin/wards/${slumData.ward}`,
                );
          ...
          if (surveyResponse.success) {
            ...
            if (surveyData.completionPercentage !== undefined) {
              ...
              if (surveyData.surveyStatus === "SUBMITTED") {
                ...
                sectionIndex = Math.floor(
                  surveyData.completionPercentage / (100 / 15),
                );
            ...
            console.log(
              "Loading basicInformation data:",
              surveyData.basicInformation,
            );
            if (surveyData.basicInformation) {
              setFormData((prev) => ({
                ...
                ageSlumYears:
                  surveyData.basicInformation.ageSlumYears || undefined,
                locationCoreOrFringe:
                  surveyData.basicInformation.locationCoreOrFringe || "",
                typeAreaSurrounding:
                  surveyData.basicInformation.typeAreaSurrounding || "",
                physicalLocationSlum:
                  surveyData.basicInformation.physicalLocationSlum || "",
              }));
            }
            ...
            if (surveyData.landStatus) {
              setFormData((prev) => ({
                ...prev,
                ownershipLandDetail:
                  surveyData.landStatus.ownershipLandDetail || "",
                ownershipLandSpecify:
                  surveyData.landStatus.ownershipLandSpecify || "",
              }));
            }

            // Handle demographic profile data
            console.log(
              "Loading demographicProfile data:",
              surveyData.demographicProfile,
            );
            if (surveyData.demographicProfile) {
              setFormData((prev) => ({
                ...prev,
                // Population data
                totalPopulationSlumSC:
                  surveyData.demographicProfile.totalPopulation?.SC || 0,
                totalPopulationSlumST:
                  surveyData.demographicProfile.totalPopulation?.ST || 0,
                totalPopulationSlumOBC:
                  surveyData.demographicProfile.totalPopulation?.OBC || 0,
                totalPopulationSlumOthers:
                  surveyData.demographicProfile.totalPopulation?.Others || 0,
                totalPopulationSlumTotal:
                  surveyData.demographicProfile.totalPopulation?.Total || 0,
                totalPopulationSlumMinorities:
                  surveyData.demographicProfile.totalPopulation?.Minorities ||
                  0,

                // BPL Population
                bplPopulationSlumSC:
                  surveyData.demographicProfile.bplPopulation?.SC || 0,
                bplPopulationSlumST:
                  surveyData.demographicProfile.bplPopulation?.ST || 0,
                bplPopulationSlumOBC:
                  surveyData.demographicProfile.bplPopulation?.OBC || 0,
                bplPopulationSlumOthers:
                  surveyData.demographicProfile.bplPopulation?.Others || 0,
                bplPopulationSlumTotal:
                  surveyData.demographicProfile.bplPopulation?.Total || 0,
                bplPopulationSlumMinorities:
                  surveyData.demographicProfile.bplPopulation?.Minorities || 0,

                // No. of Households
                noHouseholdsSlumSC:
                  surveyData.demographicProfile.numberOfHouseholds?.SC || 0,
                noHouseholdsSlumST:
                  surveyData.demographicProfile.numberOfHouseholds?.ST || 0,
                noHouseholdsSlumOBC:
                  surveyData.demographicProfile.numberOfHouseholds?.OBC || 0,
                noHouseholdsSlumOthers:
                  surveyData.demographicProfile.numberOfHouseholds?.Others || 0,
                noHouseholdsSlumTotal:
                  surveyData.demographicProfile.numberOfHouseholds?.Total || 0,
                noHouseholdsSlumMinorities:
                  surveyData.demographicProfile.numberOfHouseholds
                    ?.Minorities || 0,

                // No. of BPL Households
                noBplHouseholdsSC:
                  surveyData.demographicProfile.noBplHouseholds?.SC || 0,
                noBplHouseholdsST:
                  surveyData.demographicProfile.noBplHouseholds?.ST || 0,
                noBplHouseholdsOBC:
                  surveyData.demographicProfile.noBplHouseholds?.OBC || 0,
                noBplHouseholdsOthers:
                  surveyData.demographicProfile.noBplHouseholds?.Others || 0,
                noBplHouseholdsTotal:
                  surveyData.demographicProfile.noBplHouseholds?.Total || 0,
                noBplHouseholdsMinorities:
                  surveyData.demographicProfile.noBplHouseholds?.Minorities ||
                  0,

                // Np. of Women-headed Households
                noWomenHeadedHouseholdsSC:
                  surveyData.demographicProfile.numberOfWomenHeadedHouseholds
                    ?.SC || 0,
                noWomenHeadedHouseholdsST:
                  surveyData.demographicProfile.numberOfWomenHeadedHouseholds
                    ?.ST || 0,
                noWomenHeadedHouseholdsOBC:
                  surveyData.demographicProfile.numberOfWomenHeadedHouseholds
                    ?.OBC || 0,
                noWomenHeadedHouseholdsOthers:
                  surveyData.demographicProfile.numberOfWomenHeadedHouseholds
                    ?.Others || 0,
                noWomenHeadedHouseholdsTotal:
                  surveyData.demographicProfile.numberOfWomenHeadedHouseholds
                    ?.Total || 0,
                noWomenHeadedHouseholdsMinorities:
                  surveyData.demographicProfile.numberOfWomenHeadedHouseholds
                    ?.Minorities || 0,

                // No. of Persons older than 65 Years
                noPersonsOlder65SC:
                  surveyData.demographicProfile.noPersonsOlder65?.SC || 0,
                noPersonsOlder65ST:
                  surveyData.demographicProfile.noPersonsOlder65?.ST || 0,
                noPersonsOlder65OBC:
                  surveyData.demographicProfile.noPersonsOlder65?.OBC || 0,
                noPersonsOlder65Others:
                  surveyData.demographicProfile.noPersonsOlder65?.Others || 0,
                noPersonsOlder65Total:
                  surveyData.demographicProfile.noPersonsOlder65?.Total || 0,
                noPersonsOlder65Minorities:
                  surveyData.demographicProfile.noPersonsOlder65?.Minorities ||
                  0,

                // No. of Child Labourers
                noChildLabourersSC:
                  surveyData.demographicProfile.noChildLabourers?.SC || 0,
                noChildLabourersST:
                  surveyData.demographicProfile.noChildLabourers?.ST || 0,
                noChildLabourersOBC:
                  surveyData.demographicProfile.noChildLabourers?.OBC || 0,
                noChildLabourersOthers:
                  surveyData.demographicProfile.noChildLabourers?.Others || 0,
                noChildLabourersTotal:
                  surveyData.demographicProfile.noChildLabourers?.Total || 0,
                noChildLabourersMinorities:
                  surveyData.demographicProfile.noChildLabourers?.Minorities ||
                  0,

                // No. of Physically Challenged Persons
                noPhysicallyChallengedSC:
                  surveyData.demographicProfile.noPhysicallyChallenged?.SC || 0,
                noPhysicallyChallengedST:
                  surveyData.demographicProfile.noPhysicallyChallenged?.ST || 0,
                noPhysicallyChallengedOBC:
                  surveyData.demographicProfile.noPhysicallyChallenged?.OBC ||
                  0,
                noPhysicallyChallengedOthers:
                  surveyData.demographicProfile.noPhysicallyChallenged
                    ?.Others || 0,
                noPhysicallyChallengedTotal:
                  surveyData.demographicProfile.noPhysicallyChallenged?.Total ||
                  0,
                noPhysicallyChallengedMinorities:
                  surveyData.demographicProfile.noPhysicallyChallenged
                    ?.Minorities || 0,

                // No. of Mentally Challenged Persons
                noMentallyChallengedSC:
                  surveyData.demographicProfile.noMentallyChallenged?.SC || 0,
                noMentallyChallengedST:
                  surveyData.demographicProfile.noMentallyChallenged?.ST || 0,
                noMentallyChallengedOBC:
                  surveyData.demographicProfile.noMentallyChallenged?.OBC || 0,
                noMentallyChallengedOthers:
                  surveyData.demographicProfile.noMentallyChallenged?.Others ||
                  0,
                noMentallyChallengedTotal:
                  surveyData.demographicProfile.noMentallyChallenged?.Total ||
                  0,
                noMentallyChallengedMinorities:
                  surveyData.demographicProfile.noMentallyChallenged
                    ?.Minorities || 0,
                ...
                noHivAidsOthers:
                  surveyData.demographicProfile.noHivAids?.Others || 0,
                noHivAidsTotal:
                  surveyData.demographicProfile.noHivAids?.Total || 0,
                noHivAidsMinorities:
                  surveyData.demographicProfile.noHivAids?.Minorities || 0,

                // No. of Persons with Tuberculosis
                noTuberculosisSC:
                  surveyData.demographicProfile.noTuberculosis?.SC || 0,
                noTuberculosisST:
                  surveyData.demographicProfile.noTuberculosis?.ST || 0,
                noTuberculosisOBC:
                  surveyData.demographicProfile.noTuberculosis?.OBC || 0,
                noTuberculosisOthers:
                  surveyData.demographicProfile.noTuberculosis?.Others || 0,
                noTuberculosisTotal:
                  surveyData.demographicProfile.noTuberculosis?.Total || 0,
                noTuberculosisMinorities:
                  surveyData.demographicProfile.noTuberculosis?.Minorities || 0,

                // No. of Persons with Respiratory Diseases including Asthma
                noRespiratoryDiseasesSC:
                  surveyData.demographicProfile.noRespiratoryDiseases?.SC || 0,
                noRespiratoryDiseasesST:
                  surveyData.demographicProfile.noRespiratoryDiseases?.ST || 0,
                noRespiratoryDiseasesOBC:
                  surveyData.demographicProfile.noRespiratoryDiseases?.OBC || 0,
                noRespiratoryDiseasesOthers:
                  surveyData.demographicProfile.noRespiratoryDiseases?.Others ||
                  0,
                noRespiratoryDiseasesTotal:
                  surveyData.demographicProfile.noRespiratoryDiseases?.Total ||
                  0,
                noRespiratoryDiseasesMinorities:
                  surveyData.demographicProfile.noRespiratoryDiseases
                    ?.Minorities || 0,

                // No. of Persons with Other Chronic Diseases
                noOtherChronicDiseasesSC:
                  surveyData.demographicProfile.noOtherChronicDiseases?.SC || 0,
                noOtherChronicDiseasesST:
                  surveyData.demographicProfile.noOtherChronicDiseases?.ST || 0,
                noOtherChronicDiseasesOBC:
                  surveyData.demographicProfile.noOtherChronicDiseases?.OBC ||
                  0,
                noOtherChronicDiseasesOthers:
                  surveyData.demographicProfile.noOtherChronicDiseases
                    ?.Others || 0,
                noOtherChronicDiseasesTotal:
                  surveyData.demographicProfile.noOtherChronicDiseases?.Total ||
                  0,
                noOtherChronicDiseasesMinorities:
                  surveyData.demographicProfile.noOtherChronicDiseases
                    ?.Minorities || 0,

                // Total No. of Illiterte Persons
                totalIlliteratePersonsSC:
                  surveyData.demographicProfile.totalIlliteratePerson?.SC || 0,
                totalIlliteratePersonsST:
                  surveyData.demographicProfile.totalIlliteratePerson?.ST || 0,
                totalIlliteratePersonsOBC:
                  surveyData.demographicProfile.totalIlliteratePerson?.OBC || 0,
                totalIlliteratePersonsOthers:
                  surveyData.demographicProfile.totalIlliteratePerson?.Others ||
                  0,
                totalIlliteratePersonsTotal:
                  surveyData.demographicProfile.totalIlliteratePerson?.Total ||
                  0,
                totalIlliteratePersonsMinorities:
                  surveyData.demographicProfile.totalIlliteratePerson
                    ?.Minorities || 0,

                // No. of Male Illiterate
                noMaleIlliterateSC:
                  surveyData.demographicProfile.noMaleIlliterate?.SC || 0,
                noMaleIlliterateST:
                  surveyData.demographicProfile.noMaleIlliterate?.ST || 0,
                noMaleIlliterateOBC:
                  surveyData.demographicProfile.noMaleIlliterate?.OBC || 0,
                noMaleIlliterateOthers:
                  surveyData.demographicProfile.noMaleIlliterate?.Others || 0,
                noMaleIlliterateTotal:
                  surveyData.demographicProfile.noMaleIlliterate?.Total || 0,
                noMaleIlliterateMinorities:
                  surveyData.demographicProfile.noMaleIlliterate?.Minorities ||
                  0,

                // No. of Female Illiterate
                noFemaleIlliterateSC:
                  surveyData.demographicProfile.noFemaleIlliterate?.SC || 0,
                noFemaleIlliterateST:
                  surveyData.demographicProfile.noFemaleIlliterate?.ST || 0,
                noFemaleIlliterateOBC:
                  surveyData.demographicProfile.noFemaleIlliterate?.OBC || 0,
                noFemaleIlliterateOthers:
                  surveyData.demographicProfile.noFemaleIlliterate?.Others || 0,
                noFemaleIlliterateTotal:
                  surveyData.demographicProfile.noFemaleIlliterate?.Total || 0,
                noFemaleIlliterateMinorities:
                  surveyData.demographicProfile.noFemaleIlliterate
                    ?.Minorities || 0,

                // No. of BPL Illiterate Persons
                noBplIlliteratePersonsSC:
                  surveyData.demographicProfile.noBplIlliteratePersons?.SC || 0,
                noBplIlliteratePersonsST:
                  surveyData.demographicProfile.noBplIlliteratePersons?.ST || 0,
                noBplIlliteratePersonsOBC:
                  surveyData.demographicProfile.noBplIlliteratePersons?.OBC ||
                  0,
                noBplIlliteratePersonsOthers:
                  surveyData.demographicProfile.noBplIlliteratePersons
                    ?.Others || 0,
                noBplIlliteratePersonsTotal:
                  surveyData.demographicProfile.noBplIlliteratePersons?.Total ||
                  0,
                noBplIlliteratePersonsMinorities:
                  surveyData.demographicProfile.noBplIlliteratePersons
                    ?.Minorities || 0,

                // No. of Male BPL Illiterate
                noMaleBplIlliterateSC:
                  surveyData.demographicProfile.noMaleBplIlliterate?.SC || 0,
                noMaleBplIlliterateST:
                  surveyData.demographicProfile.noMaleBplIlliterate?.ST || 0,
                noMaleBplIlliterateOBC:
                  surveyData.demographicProfile.noMaleBplIlliterate?.OBC || 0,
                noMaleBplIlliterateOthers:
                  surveyData.demographicProfile.noMaleBplIlliterate?.Others ||
                  0,
                noMaleBplIlliterateTotal:
                  surveyData.demographicProfile.noMaleBplIlliterate?.Total || 0,
                noMaleBplIlliterateMinorities:
                  surveyData.demographicProfile.noMaleBplIlliterate
                    ?.Minorities || 0,

                // No. of Female BPL Illiterate
                noFemaleBplIlliterateSC:
                  surveyData.demographicProfile.noFemaleBplIlliterate?.SC || 0,
                noFemaleBplIlliterateST:
                  surveyData.demographicProfile.noFemaleBplIlliterate?.ST || 0,
                noFemaleBplIlliterateOBC:
                  surveyData.demographicProfile.noFemaleBplIlliterate?.OBC || 0,
                noFemaleBplIlliterateOthers:
                  surveyData.demographicProfile.noFemaleBplIlliterate?.Others ||
                  0,
                noFemaleBplIlliterateTotal:
                  surveyData.demographicProfile.noFemaleBplIlliterate?.Total ||
                  0,
                noFemaleBplIlliterateMinorities:
                  surveyData.demographicProfile.noFemaleBplIlliterate
                    ?.Minorities || 0,

                // School Dropouts - Male
                noMaleSchoolDropoutsSC:
                  surveyData.demographicProfile.noMaleSchoolDropouts?.SC || 0,
                noMaleSchoolDropoutsST:
                  surveyData.demographicProfile.noMaleSchoolDropouts?.ST || 0,
                noMaleSchoolDropoutsOBC:
                  surveyData.demographicProfile.noMaleSchoolDropouts?.OBC || 0,
                noMaleSchoolDropoutsOthers:
                  surveyData.demographicProfile.noMaleSchoolDropouts?.Others ||
                  0,
                noMaleSchoolDropoutsTotal:
                  surveyData.demographicProfile.noMaleSchoolDropouts?.Total ||
                  0,
                noMaleSchoolDropoutsMinorities:
                  surveyData.demographicProfile.noMaleSchoolDropouts
                    ?.Minorities || 0,

                // School Dropouts - Female
                noFemaleSchoolDropoutsSC:
                  surveyData.demographicProfile.noFemaleSchoolDropouts?.SC || 0,
                noFemaleSchoolDropoutsST:
                  surveyData.demographicProfile.noFemaleSchoolDropouts?.ST || 0,
                noFemaleSchoolDropoutsOBC:
                  surveyData.demographicProfile.noFemaleSchoolDropouts?.OBC ||
                  0,
                noFemaleSchoolDropoutsOthers:
                  surveyData.demographicProfile.noFemaleSchoolDropouts
                    ?.Others || 0,
                noFemaleSchoolDropoutsTotal:
                  surveyData.demographicProfile.noFemaleSchoolDropouts?.Total ||
                  0,
                noFemaleSchoolDropoutsMinorities:
                  surveyData.demographicProfile.noFemaleSchoolDropouts
                    ?.Minorities || 0,
              }));
            }
            ...
            if (surveyData.physicalInfrastructure) {
              setFormData((prev) => ({
                ...
                sourceDrinkingWater: {
                  individualTap:
                    surveyData.physicalInfrastructure.sourceDrinkingWater
                      ?.individualTap || 0,
                  tubewellBorewellHandpump:
                    surveyData.physicalInfrastructure.sourceDrinkingWater
                      ?.tubewellBorewellHandpump || 0,
                  publicTap:
                    surveyData.physicalInfrastructure.sourceDrinkingWater
                ...
                connectivityCityWaterSupply:
                  surveyData.physicalInfrastructure
                    .connectivityCityWaterSupply || "",
                drainageSewerageFacility:
                  surveyData.physicalInfrastructure.drainageSewerageFacility ||
                  "",
                connectivityStormWaterDrainage:
                  surveyData.physicalInfrastructure
                    .connectivityStormWaterDrainage || "",
                connectivitySewerageSystem:
                  surveyData.physicalInfrastructure
                    .connectivitySewerageSystem || "",
                proneToFlooding:
                  surveyData.physicalInfrastructure.proneToFlooding || "",
                latrineFacility:
                  surveyData.physicalInfrastructure.latrineFacility || {},
                frequencyOfGarbageDisposal:
                  surveyData.physicalInfrastructure.solidWasteManagement
                    ?.frequencyOfGarbageDisposal || "",
                arrangementForGarbageDisposal:
                  surveyData.physicalInfrastructure.solidWasteManagement
                    ?.arrangementForGarbageDisposal || "",
                frequencyOfClearanceOfOpenDrains:
                  surveyData.physicalInfrastructure.solidWasteManagement
                    ?.frequencyOfClearanceOfOpenDrains || "",
                streetLightAvailable:
                  surveyData.physicalInfrastructure.streetLightAvailable || "",
                approachRoadType:
                  surveyData.physicalInfrastructure.approachRoadType || "",
                distanceToNearestMotorableRoad:
                  surveyData.physicalInfrastructure
                    .distanceToNearestMotorableRoad || "",
                internalRoadType:
                  surveyData.physicalInfrastructure.internalRoadType || "",
              }));
            }
            ...
            if (surveyData.healthFacilities) {
              setFormData((prev) => ({
                ...prev,
                urbanHealthPost:
                  surveyData.healthFacilities.urbanHealthPost || "",
                primaryHealthCentre:
                  surveyData.healthFacilities.primaryHealthCentre || "",
                governmentHospital:
                  surveyData.healthFacilities.governmentHospital || "",
                maternityCentre:
                  surveyData.healthFacilities.maternityCentre || "",
                privateClinic: surveyData.healthFacilities.privateClinic || "",
                rmp: surveyData.healthFacilities.rmp || "",
                ayurvedicDoctor:
                  surveyData.healthFacilities.ayurvedicDoctor || "",
              }));
            }
            ...
            if (surveyData.socialDevelopment) {
              setFormData((prev) => ({
                ...
                livelihoodProductionCentre:
                  surveyData.socialDevelopment.livelihoodProductionCentre || 0,
                vocationalTrainingCentre:
                  surveyData.socialDevelopment.vocationalTrainingCentre || 0,
                streetChildrenRehabilitationCentre:
                  surveyData.socialDevelopment
                    .streetChildrenRehabilitationCentre || 0,
                ...
                oldAgePensionsHolders:
                  surveyData.socialDevelopment.oldAgePensionsHolders || 0,
                widowPensionsHolders:
                  surveyData.socialDevelopment.widowPensionsHolders || 0,
                disabledPensionsHolders:
                  surveyData.socialDevelopment.disabledPensionsHolders || 0,
                generalInsuranceCovered:
                  surveyData.socialDevelopment.generalInsuranceCovered || 0,
                healthInsuranceCovered:
                  surveyData.socialDevelopment.healthInsuranceCovered || 0,
                selfHelpGroups:
                  surveyData.socialDevelopment.selfHelpGroups || 0,
                thriftCreditSocieties:
                  surveyData.socialDevelopment.thriftCreditSocieties || 0,
                slumDwellersAssociation:
                  surveyData.socialDevelopment.slumDwellersAssociation || "",
                youthAssociations:
                  surveyData.socialDevelopment.youthAssociations || 0,
                womensAssociations:
                  surveyData.socialDevelopment.womensAssociations || 0,
              }));
            }
            ...
            if (surveyData.housingStatus) {
              setFormData((prev) => ({
                ...prev,
                dwellingUnitsPucca:
                  surveyData.housingStatus.dwellingUnitsPucca || 0,
                dwellingUnitsSemiPucca:
                  surveyData.housingStatus.dwellingUnitsSemiPucca || 0,
                dwellingUnitsKatcha:
                  surveyData.housingStatus.dwellingUnitsKatcha || 0,
                dwellingUnitsTotal:
                  surveyData.housingStatus.dwellingUnitsTotal || 0,
                dwellingUnitsWithElectricityPucca:
                  surveyData.housingStatus.dwellingUnitsWithElectricityPucca ||
                  0,
                dwellingUnitsWithElectricitySemiPucca:
                  surveyData.housingStatus
                    .dwellingUnitsWithElectricitySemiPucca || 0,
                dwellingUnitsWithElectricityKatcha:
                  surveyData.housingStatus.dwellingUnitsWithElectricityKatcha ||
                  0,
                dwellingUnitsWithElectricityTotal:
                  surveyData.housingStatus.dwellingUnitsWithElectricityTotal ||
                  0,
                landTenureWithPatta:
                  surveyData.housingStatus.landTenureWithPatta || 0,
                landTenurePossessionCertificate:
                  surveyData.housingStatus.landTenurePossessionCertificate || 0,
                landTenureEncroachedPrivate:
                  surveyData.housingStatus.landTenureEncroachedPrivate || 0,
                landTenureEncroachedPublic:
                  surveyData.housingStatus.landTenureEncroachedPublic || 0,
                landTenureOnRent:
                  surveyData.housingStatus.landTenureOnRent || 0,
            ...
            if (surveyData.educationFacilities) {
              setFormData((prev) => ({
                ...prev,
                anganwadiUnderIcds: surveyData.educationFacilities
                  .anganwadiUnderIcds || { option: "", distance: null },
                municipalPreschool: surveyData.educationFacilities
                  .municipalPreschool || { option: "", distance: null },
                privatePreschool: surveyData.educationFacilities
                  .privatePreschool || { option: "", distance: null },
                municipalPrimarySchool: surveyData.educationFacilities
                  .municipalPrimarySchool || { option: "", distance: null },
                stateGovtPrimarySchool: surveyData.educationFacilities
                  .stateGovtPrimarySchool || { option: "", distance: null },
                privatePrimarySchool: surveyData.educationFacilities
                  .privatePrimarySchool || { option: "", distance: null },
                municipalHighSchool: surveyData.educationFacilities
                  .municipalHighSchool || { option: "", distance: null },
                stateGovtHighSchool: surveyData.educationFacilities
                  .stateGovtHighSchool || { option: "", distance: null },
                privateHighSchool: surveyData.educationFacilities
                  .privateHighSchool || { option: "", distance: null },
                adultEducationCentre: surveyData.educationFacilities
                  .adultEducationCentre || { option: "", distance: null },
                nonFormalEducationCentre: surveyData.educationFacilities
                  .nonFormalEducationCentre || { option: "", distance: null },
              }));
            }
            ...
            if (surveyData.demographicProfile) {
              ...
              setFormData((prev) => ({
                ...
                // Male Illiterate
                noMaleIlliterateSC:
                  surveyData.demographicProfile.maleIlliterate?.SC || 0,
                noMaleIlliterateST:
                  surveyData.demographicProfile.maleIlliterate?.ST || 0,
                noMaleIlliterateOBC:
                  surveyData.demographicProfile.maleIlliterate?.OBC || 0,
                noMaleIlliterateOthers:
                  surveyData.demographicProfile.maleIlliterate?.Others || 0,
                noMaleIlliterateTotal:
                  surveyData.demographicProfile.maleIlliterate?.Total || 0,
                noMaleIlliterateMinorities:
                  surveyData.demographicProfile.maleIlliterate?.Minorities || 0,

                // Female Illiterate
                noFemaleIlliterateSC:
                  surveyData.demographicProfile.femaleIlliterate?.SC || 0,
                noFemaleIlliterateST:
                  surveyData.demographicProfile.femaleIlliterate?.ST || 0,
                noFemaleIlliterateOBC:
                  surveyData.demographicProfile.femaleIlliterate?.OBC || 0,
                noFemaleIlliterateOthers:
                  surveyData.demographicProfile.femaleIlliterate?.Others || 0,
                noFemaleIlliterateTotal:
                  surveyData.demographicProfile.femaleIlliterate?.Total || 0,
                noFemaleIlliterateMinorities:
                  surveyData.demographicProfile.femaleIlliterate?.Minorities ||
                  0,

                // BPL Illiterate Persons
                noBplIlliteratePersonsSC:
                  surveyData.demographicProfile.bplIlliteratePerson?.SC || 0,
                noBplIlliteratePersonsST:
                  surveyData.demographicProfile.bplIlliteratePerson?.ST || 0,
                noBplIlliteratePersonsOBC:
                  surveyData.demographicProfile.bplIlliteratePerson?.OBC || 0,
                noBplIlliteratePersonsOthers:
                  surveyData.demographicProfile.bplIlliteratePerson?.Others ||
                  0,
                noBplIlliteratePersonsTotal:
                  surveyData.demographicProfile.bplIlliteratePerson?.Total || 0,
                noBplIlliteratePersonsMinorities:
                  surveyData.demographicProfile.bplIlliteratePerson
                    ?.Minorities || 0,

                // Male BPL Illiterate
                noMaleBplIlliterateSC:
                  surveyData.demographicProfile.maleBplIlliterate?.SC || 0,
                noMaleBplIlliterateST:
                  surveyData.demographicProfile.maleBplIlliterate?.ST || 0,
                noMaleBplIlliterateOBC:
                  surveyData.demographicProfile.maleBplIlliterate?.OBC || 0,
                noMaleBplIlliterateOthers:
                  surveyData.demographicProfile.maleBplIlliterate?.Others || 0,
                noMaleBplIlliterateTotal:
                  surveyData.demographicProfile.maleBplIlliterate?.Total || 0,
                noMaleBplIlliterateMinorities:
                  surveyData.demographicProfile.maleBplIlliterate?.Minorities ||
                  0,

                // Female BPL Illiterate
                noFemaleBplIlliterateSC:
                  surveyData.demographicProfile.femaleBplIlliterate?.SC || 0,
                noFemaleBplIlliterateST:
                  surveyData.demographicProfile.femaleBplIlliterate?.ST || 0,
                noFemaleBplIlliterateOBC:
                  surveyData.demographicProfile.femaleBplIlliterate?.OBC || 0,
                noFemaleBplIlliterateOthers:
                  surveyData.demographicProfile.femaleBplIlliterate?.Others ||
                  0,
                noFemaleBplIlliterateTotal:
                  surveyData.demographicProfile.femaleBplIlliterate?.Total || 0,
                noFemaleBplIlliterateMinorities:
                  surveyData.demographicProfile.femaleBplIlliterate
                    ?.Minorities || 0,

                // School Dropouts Male
                schoolDropoutsMaleSC:
                  surveyData.demographicProfile.schoolDropoutsMale?.SC || 0,
                schoolDropoutsMaleST:
                  surveyData.demographicProfile.schoolDropoutsMale?.ST || 0,
                schoolDropoutsMaleOBC:
                  surveyData.demographicProfile.schoolDropoutsMale?.OBC || 0,
                schoolDropoutsMaleOthers:
                  surveyData.demographicProfile.schoolDropoutsMale?.Others || 0,
                schoolDropoutsMaleTotal:
                  surveyData.demographicProfile.schoolDropoutsMale?.Total || 0,
                schoolDropoutsMaleMinorities:
                  surveyData.demographicProfile.schoolDropoutsMale
                    ?.Minorities || 0,

                // School Dropouts Female
                schoolDropoutsFemaleSC:
                  surveyData.demographicProfile.schoolDropoutsFemale?.SC || 0,
                schoolDropoutsFemaleST:
                  surveyData.demographicProfile.schoolDropoutsFemale?.ST || 0,
                schoolDropoutsFemaleOBC:
                  surveyData.demographicProfile.schoolDropoutsFemale?.OBC || 0,
                schoolDropoutsFemaleOthers:
                  surveyData.demographicProfile.schoolDropoutsFemale?.Others ||
                  0,
                schoolDropoutsFemaleTotal:
                  surveyData.demographicProfile.schoolDropoutsFemale?.Total ||
                  0,
                schoolDropoutsFemaleMinorities:
                  surveyData.demographicProfile.schoolDropoutsFemale
                    ?.Minorities || 0,

                // Number of BPL Households (from demographicProfile.numberOfBplHouseholds)
                noBplHouseholdsSC:
                  surveyData.demographicProfile.numberOfBplHouseholds?.SC || 0,
                noBplHouseholdsST:
                  surveyData.demographicProfile.numberOfBplHouseholds?.ST || 0,
                noBplHouseholdsOBC:
                  surveyData.demographicProfile.numberOfBplHouseholds?.OBC || 0,
                noBplHouseholdsOthers:
                  surveyData.demographicProfile.numberOfBplHouseholds?.Others ||
                  0,
                noBplHouseholdsTotal:
                  surveyData.demographicProfile.numberOfBplHouseholds?.Total ||
                  0,
                noBplHouseholdsMinorities:
                  surveyData.demographicProfile.numberOfBplHouseholds
                    ?.Minorities || 0,
                ...
                noPersonsOlder65:
                  surveyData.demographicProfile.personsOlderThan65Years
                    ?.Total || 0,
                noPersonsOlder65SC:
                  surveyData.demographicProfile.personsOlderThan65Years?.SC ||
                  0,
                noPersonsOlder65ST:
                  surveyData.demographicProfile.personsOlderThan65Years?.ST ||
                  0,
                noPersonsOlder65OBC:
                  surveyData.demographicProfile.personsOlderThan65Years?.OBC ||
                  0,
                noPersonsOlder65Others:
                  surveyData.demographicProfile.personsOlderThan65Years
                    ?.Others || 0,
                ...
                // Child Labourers
                noChildLabourers:
                  surveyData.demographicProfile.childLabourers?.Total || 0,
                noChildLabourersSC:
                  surveyData.demographicProfile.childLabourers?.SC || 0,
                noChildLabourersST:
                  surveyData.demographicProfile.childLabourers?.ST || 0,
                noChildLabourersOBC:
                  surveyData.demographicProfile.childLabourers?.OBC || 0,
                noChildLabourersOthers:
                  surveyData.demographicProfile.childLabourers?.Others || 0,
                noChildLabourersTotal:
                  surveyData.demographicProfile.childLabourers?.Total || 0,
                noChildLabourersMinorities:
                  surveyData.demographicProfile.childLabourers?.Minorities || 0,

                // Physically Challenged Persons
                noPhysicallyChallenged:
                  surveyData.demographicProfile.physicallyChallengedPersons
                    ?.Total || 0,
                noPhysicallyChallengedSC:
                  surveyData.demographicProfile.physicallyChallengedPersons
                    ?.SC || 0,
                ...
                noMentallyChallenged:
                  surveyData.demographicProfile.mentallyChallengedPersons
                    ?.Total || 0,
                noMentallyChallengedSC:
                  surveyData.demographicProfile.mentallyChallengedPersons?.SC ||
                  0,
                noMentallyChallengedST:
                  surveyData.demographicProfile.mentallyChallengedPersons?.ST ||
                  0,
                noMentallyChallengedOBC:
                  surveyData.demographicProfile.mentallyChallengedPersons
                    ?.OBC || 0,
                ...
                // HIV/AIDS
                noPersonsHivaids:
                  surveyData.demographicProfile.personsWithHivAids?.Total || 0,
                noPersonsHivaidsSC:
                  surveyData.demographicProfile.personsWithHivAids?.SC || 0,
                noPersonsHivaidsST:
                  surveyData.demographicProfile.personsWithHivAids?.ST || 0,
                noPersonsHivaidsOBC:
                  surveyData.demographicProfile.personsWithHivAids?.OBC || 0,
                noPersonsHivaidsOthers:
                  surveyData.demographicProfile.personsWithHivAids?.Others || 0,
                noPersonsHivaidsTotal:
                  surveyData.demographicProfile.personsWithHivAids?.Total || 0,
                noPersonsHivaidsMinorities:
                  surveyData.demographicProfile.personsWithHivAids
                    ?.Minorities || 0,

                // Tuberculosis
                noPersonsTuberculosis:
                  surveyData.demographicProfile.personsWithTuberculosis
                    ?.Total || 0,
                noPersonsTuberculosisSC:
                  surveyData.demographicProfile.personsWithTuberculosis?.SC ||
                  0,
                noPersonsTuberculosisST:
                  surveyData.demographicProfile.personsWithTuberculosis?.ST ||
                  0,
                noPersonsTuberculosisOBC:
                  surveyData.demographicProfile.personsWithTuberculosis?.OBC ||
                  0,
                noPersonsTuberculosisOthers:
                  surveyData.demographicProfile.personsWithTuberculosis
                    ?.Others || 0,
                noPersonsTuberculosisTotal:
                  surveyData.demographicProfile.personsWithTuberculosis
                    ?.Total || 0,
                noPersonsTuberculosisMinorities:
                  surveyData.demographicProfile.personsWithTuberculosis
                    ?.Minorities || 0,

                // Respiratory Diseases
                noPersonsRespiratory:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.Total || 0,
                noPersonsRespiratorySC:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.SC || 0,
                noPersonsRespiratoryST:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.ST || 0,
                noPersonsRespiratoryOBC:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.OBC || 0,
                noPersonsRespiratoryOthers:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.Others || 0,
                noPersonsRespiratoryTotal:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.Total || 0,
                noPersonsRespiratoryMinorities:
                  surveyData.demographicProfile.personsWithRespiratoryDiseases
                    ?.Minorities || 0,

                // Other Chronic Diseases
                noPersonsOtherChronic:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.Total || 0,
                noPersonsOtherChronicSC:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.SC || 0,
                noPersonsOtherChronicST:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.ST || 0,
                noPersonsOtherChronicOBC:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.OBC || 0,
                noPersonsOtherChronicOthers:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.Others || 0,
                noPersonsOtherChronicTotal:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.Total || 0,
                noPersonsOtherChronicMinorities:
                  surveyData.demographicProfile.personsWithOtherChronicDiseases
                    ?.Minorities || 0,
              }));
            }
            ...
            if (surveyData.employmentAndOccupation) {
              setFormData((prev) => ({
                ...
                occupationalStatus: {
                  selfEmployed:
                    surveyData.employmentAndOccupation.selfEmployed || 0,
                  salaried: surveyData.employmentAndOccupation.salaried || 0,
                  regularWage:
                    surveyData.employmentAndOccupation.regularWage || 0,
                  casualLabour:
                    surveyData.employmentAndOccupation.casualLabour || 0,
            ...
            if (surveyData.additionalInfrastructure) {
              setFormData((prev) => ({
                ...
                waterSupplyPipelinesExisting:
                  surveyData.additionalInfrastructure?.waterSupply?.pipelines
                    ?.existing || 0,
                waterSupplyPipelinesAdditional:
                  surveyData.additionalInfrastructure.waterSupply?.pipelines
                    ?.additionalRequirement || 0,
                waterSupplyPipelinesCost:
                  surveyData.additionalInfrastructure.waterSupply?.pipelines
                    ?.estimatedCost || 0,

                waterSupplyIndividualTapsExisting:
                  surveyData.additionalInfrastructure.waterSupply
                    ?.individualTaps?.existing || 0,
                waterSupplyIndividualTapsAdditional:
                  surveyData.additionalInfrastructure.waterSupply
                    ?.individualTaps?.additionalRequirement || 0,
                waterSupplyIndividualTapsCost:
                  surveyData.additionalInfrastructure.waterSupply
                    ?.individualTaps?.estimatedCost || 0,

                waterSupplyBorewellsExisting:
                  surveyData.additionalInfrastructure.waterSupply?.borewells
                    ?.existing || 0,
                waterSupplyBorewellsAdditional:
                  surveyData.additionalInfrastructure.waterSupply?.borewells
                    ?.additionalRequirement || 0,
                waterSupplyBorewellsCost:
                  surveyData.additionalInfrastructure.waterSupply?.borewells
                    ?.estimatedCost || 0,

                waterSupplyConnectivityTrunkLinesExisting:
                  surveyData.additionalInfrastructure.waterSupply
                    ?.connectivityToTrunkLines?.existing || 0,
                waterSupplyConnectivityTrunkLinesAdditional:
                  surveyData.additionalInfrastructure.waterSupply
                    ?.connectivityToTrunkLines?.additionalRequirement || 0,
                waterSupplyConnectivityTrunkLinesCost:
                  surveyData.additionalInfrastructure.waterSupply
                    ?.connectivityToTrunkLines?.estimatedCost || 0,

                // Drainage/Sewerage
                drainageStormwaterDrainageExisting:
                  surveyData.additionalInfrastructure.drainageewerage
                    ?.stormwaterDrainage?.existing || 0,
                drainageStormwaterDrainageAdditional:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.stormwaterDrainage?.additionalRequirement || 0,
                drainageStormwaterDrainageCost:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.stormwaterDrainage?.estimatedCost || 0,

                drainageConnectivityMainDrainsExisting:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.connectivityToMainDrains?.existing || 0,
                drainageConnectivityMainDrainsAdditional:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.connectivityToMainDrains?.additionalRequirement || 0,
                drainageConnectivityMainDrainsCost:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.connectivityToMainDrains?.estimatedCost || 0,

                drainageSewerLinesExisting:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.sewerLines?.existing || 0,
                drainageSewerLinesAdditional:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.sewerLines?.additionalRequirement || 0,
                drainageSewerLinesCost:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.sewerLines?.estimatedCost || 0,

                drainageConnectivityTrunkSewersExisting:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.connectivityToTrunkSewers?.existing || 0,
                drainageConnectivityTrunkSewersAdditional:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.connectivityToTrunkSewers?.additionalRequirement || 0,
                drainageConnectivityTrunkSewersCost:
                  surveyData.additionalInfrastructure.drainageSewerage
                    ?.connectivityToTrunkSewers?.estimatedCost || 0,

                // Roads
                roadsInternalRoadsCCExisting:
                  surveyData.additionalInfrastructure.roads?.internalRoadsCC
                    ?.existing || 0,
                roadsInternalRoadsCCAdditional:
                  surveyData.additionalInfrastructure.roads?.internalRoadsCC
                    ?.additionalRequirement || 0,
                roadsInternalRoadsCCCost:
                  surveyData.additionalInfrastructure.roads?.internalRoadsCC
                    ?.estimatedCost || 0,

                roadsInternalRoadsBTExisting:
                  surveyData.additionalInfrastructure.roads?.internalRoadsBT
                    ?.existing || 0,
                roadsInternalRoadsBTAdditional:
                  surveyData.additionalInfrastructure.roads?.internalRoadsBT
                    ?.additionalRequirement || 0,
                roadsInternalRoadsBTCost:
                  surveyData.additionalInfrastructure.roads?.internalRoadsBT
                    ?.estimatedCost || 0,

                roadsInternalRoadsOthersExisting:
                  surveyData.additionalInfrastructure.roads?.internalRoadsOthers
                    ?.existing || 0,
                roadsInternalRoadsOthersAdditional:
                  surveyData.additionalInfrastructure.roads?.internalRoadsOthers
                    ?.additionalRequirement || 0,
                roadsInternalRoadsOthersCost:
                  surveyData.additionalInfrastructure.roads?.internalRoadsOthers
                    ?.estimatedCost || 0,

                roadsApproachRoadsCCExisting:
                  surveyData.additionalInfrastructure.roads?.approachRoadsCC
                    ?.existing || 0,
                roadsApproachRoadsCCAdditional:
                  surveyData.additionalInfrastructure.roads?.approachRoadsCC
                    ?.additionalRequirement || 0,
                roadsApproachRoadsCCCost:
                  surveyData.additionalInfrastructure.roads?.approachRoadsCC
                    ?.estimatedCost || 0,

                roadsApproachRoadsOthersExisting:
                  surveyData.additionalInfrastructure.roads?.approachRoadsOthers
                    ?.existing || 0,
                roadsApproachRoadsOthersAdditional:
                  surveyData.additionalInfrastructure.roads?.approachRoadsOthers
                    ?.additionalRequirement || 0,
                roadsApproachRoadsOthersCost:
                  surveyData.additionalInfrastructure.roads?.approachRoadsOthers
                    ?.estimatedCost || 0,

                // Street Lighting
                streetLightingPolesExisting:
                  surveyData.additionalInfrastructure.streetLighting?.poles
                    ?.existing || 0,
                streetLightingPolesAdditional:
                  surveyData.additionalInfrastructure.streetLighting?.poles
                    ?.additionalRequirement || 0,
                streetLightingPolesCost:
                  surveyData.additionalInfrastructure.streetLighting?.poles
                    ?.estimatedCost || 0,

                streetLightingLightsExisting:
                  surveyData.additionalInfrastructure.streetLighting?.lights
                    ?.existing || 0,
                streetLightingLightsAdditional:
                  surveyData.additionalInfrastructure.streetLighting?.lights
                    ?.additionalRequirement || 0,
                streetLightingLightsCost:
                  surveyData.additionalInfrastructure.streetLighting?.lights
                    ?.estimatedCost || 0,

                // Sanitation
                sanitationIndividualToiletsExisting:
                  surveyData.additionalInfrastructure.sanitation
                    ?.individualToilets?.existing || 0,
                sanitationIndividualToiletsAdditional:
                  surveyData.additionalInfrastructure.sanitation
                    ?.individualToilets?.additionalRequirement || 0,
                sanitationIndividualToiletsCost:
                  surveyData.additionalInfrastructure.sanitation
                    ?.individualToilets?.estimatedCost || 0,

                sanitationCommunityToiletsExisting:
                  surveyData.additionalInfrastructure.sanitation
                    ?.communityToilets?.existing || 0,
                sanitationCommunityToiletsAdditional:
                  surveyData.additionalInfrastructure.sanitation
                    ?.communityToilets?.additionalRequirement || 0,
                sanitationCommunityToiletsCost:
                  surveyData.additionalInfrastructure.sanitation
                    ?.communityToilets?.estimatedCost || 0,

                sanitationSeatsCommunityToiletsExisting:
                  surveyData.additionalInfrastructure.sanitation
                    ?.seatsInCommunityToilets?.existing || 0,
                sanitationSeatsCommunityToiletsAdditional:
                  surveyData.additionalInfrastructure.sanitation
                    ?.seatsInCommunityToilets?.additionalRequirement || 0,
                sanitationSeatsCommunityToiletsCost:
                  surveyData.additionalInfrastructure.sanitation
                    ?.seatsInCommunityToilets?.estimatedCost || 0,

                sanitationDumperBinsExisting:
                  surveyData.additionalInfrastructure.sanitation?.dumperBins
                    ?.existing || 0,
                sanitationDumperBinsAdditional:
                  surveyData.additionalInfrastructure.sanitation?.dumperBins
                    ?.additionalRequirement || 0,
                sanitationDumperBinsCost:
                  surveyData.additionalInfrastructure.sanitation?.dumperBins
                    ?.estimatedCost || 0,

                // Community Facilities
                communityHallsExisting:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.communityHalls?.existing || 0,
                communityHallsAdditional:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.communityHalls?.additionalRequirement || 0,
                communityHallsCost:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.communityHalls?.estimatedCost || 0,

                communityLivelihoodCentresExisting:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.livelihoodCentres?.existing || 0,
                communityLivelihoodCentresAdditional:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.livelihoodCentres?.additionalRequirement || 0,
                communityLivelihoodCentresCost:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.livelihoodCentres?.estimatedCost || 0,

                communityAnganwadisExisting:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.anganwadis?.existing || 0,
                communityAnganwadisAdditional:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.anganwadis?.additionalRequirement || 0,
                communityAnganwadisCost:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.anganwadis?.estimatedCost || 0,

                communityPrimarySchoolsExisting:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.primarySchools?.existing || 0,
                communityPrimarySchoolsAdditional:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.primarySchools?.additionalRequirement || 0,
                communityPrimarySchoolsCost:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.primarySchools?.estimatedCost || 0,

                communityHealthCentresExisting:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.healthCentres?.existing || 0,
                communityHealthCentresAdditional:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.healthCentres?.additionalRequirement || 0,
                communityHealthCentresCost:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.healthCentres?.estimatedCost || 0,
                ...
                communityOthersAdditional:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.others?.additionalRequirement || 0,
                communityOthersCost:
                  surveyData.additionalInfrastructure.communityFacilities
                    ?.others?.estimatedCost || 0,

                // Standalone Infrastructure Requirements
                electricityExisting:
                  surveyData.additionalInfrastructure
                    ?.standaloneInfrastructureRequirements?.electricity
                    ?.existing || 0,
                ...
                healthcareExisting:
                  surveyData.additionalInfrastructure
                    ?.standaloneInfrastructureRequirements?.healthCare
                    ?.existing || 0,
                ...
                toiletsExisting:
                  surveyData.additionalInfrastructure
                    ?.standaloneInfrastructureRequirements?.toilets?.existing ||
                  0,
                toiletsAdditional:
                  surveyData.additionalInfrastructure
                    ?.standaloneInfrastructureRequirements?.toilets
                    ?.additionalRequirement || 0,
                toiletsCost:
                  surveyData.additionalInfrastructure
                    ?.standaloneInfrastructureRequirements?.toilets
            ...
            if (surveyData.generalInformation) {
              setFormData((prev) => ({
                ...
                cityTownNoHouseholds:
                  surveyData.generalInformation.cityTownNoHouseholds || 0,
              }));
            }
            ...
            if (surveyData.cityTownSlumProfile) {
              setFormData((prev) => ({
                ...
                noSlumHouseholds:
                  slum?.totalHouseholds !== undefined
                    ? slum.totalHouseholds
                    : surveyData.cityTownSlumProfile.noSlumHouseholds || 0,
                // These are calculated from household surveys and stored in the survey
                slumPopulation:
                  surveyData.cityTownSlumProfile.slumPopulation || 0,
                bplPopulation:
                  surveyData.cityTownSlumProfile.bplPopulation || 0,
                bplHouseholds:
                  surveyData.cityTownSlumProfile.bplHouseholds || 0,
              }));
            }
            ...
            if (surveyData.surveyOperation) {
              setFormData((prev) => ({
                ...prev,
                surveyorName:
                  user?.name || surveyData.surveyOperation.surveyorName || "",
            ...
            if (surveyData.cityTownSlumProfile) {
              setFormData((prev) => ({
                ...prev,
                areaSqMtrs:
                  surveyData.cityTownSlumProfile.areaSqMtrs ||
                  prev.areaSqMtrs ||
                  0,
              }));
            }
  ...
  const handleNestedInputChange = (
    ...
    setFormData((prev) => ({
      ...prev,
      [parentField]: {
        ...((prev[parentField as keyof SlumSurveyForm] as unknown as Record<
          string,
  ...
  const validateForm = (): FieldError[] => {
    ...
    if (
      formData.cityTownNoHouseholds === undefined ||
      formData.cityTownNoHouseholds === null ||
      isNaN(formData.cityTownNoHouseholds) ||
      formData.cityTownNoHouseholds < 0
    ) {
      newErrors.push({
        field: "cityTownNoHouseholds",
        message: "City/Town No. of Households is required",
      });
    }

    // Part B - City/Town Slum Profile
    if (
      formData.slumPopulation === undefined ||
      formData.slumPopulation === null ||
      isNaN(formData.slumPopulation) ||
      formData.slumPopulation < 0
    ) {
      newErrors.push({
        field: "slumPopulation",
        message: "Slum Population is required",
      });
    }
    if (
      formData.noSlumHouseholds === undefined ||
      formData.noSlumHouseholds === null ||
      isNaN(formData.noSlumHouseholds) ||
      formData.noSlumHouseholds < 0
    ) {
      newErrors.push({
        field: "noSlumHouseholds",
        message: "No. of Slum Households is required",
      });
    }
    if (
      formData.bplPopulation === undefined ||
      formData.bplPopulation === null ||
      isNaN(formData.bplPopulation) ||
      formData.bplPopulation < 0
    ) {
      newErrors.push({
        field: "bplPopulation",
        message: "BPL Population is required",
      });
    }
    if (
      formData.bplHouseholds === undefined ||
      formData.bplHouseholds === null ||
      isNaN(formData.bplHouseholds) ||
      formData.bplHouseholds < 0
    ) {
      newErrors.push({
        field: "bplHouseholds",
        message: "No. of BPL Households is required",
      });
    }
    ...
    if (
      formData.ageSlumYears === undefined ||
      formData.ageSlumYears === null ||
      isNaN(formData.ageSlumYears) ||
      formData.ageSlumYears < 0
    ) {
      newErrors.push({
        field: "ageSlumYears",
        message: "Age of Slum (Years) is required",
      });
    }
    if (!formData.locationCoreOrFringe) {
      newErrors.push({
        field: "locationCoreOrFringe",
        message: "Location - Core City/Town or Fringe Area is required",
      });
    }
    if (!formData.typeAreaSurrounding) {
      newErrors.push({
        field: "typeAreaSurrounding",
        message: "Type of Area Surrounding is required",
      });
    }
    if (!formData.physicalLocationSlum) {
      newErrors.push({
        field: "physicalLocationSlum",
        message: "Physical Location of Slum is required",
      });
    }
    ...
    if (!formData.ownershipLandDetail) {
      newErrors.push({
        field: "ownershipLandDetail",
        message: "Ownership of Land is required",
      });
    }

    // Part H - Housing Status
    if (
      formData.dwellingUnitsPucca === undefined ||
      formData.dwellingUnitsPucca === null ||
      isNaN(formData.dwellingUnitsPucca) ||
      formData.dwellingUnitsPucca < 0
    ) {
      newErrors.push({
        field: "dwellingUnitsPucca",
        message: "Dwelling Units - Pucca is required",
      });
    }
    if (
      formData.dwellingUnitsSemiPucca === undefined ||
      formData.dwellingUnitsSemiPucca === null ||
      isNaN(formData.dwellingUnitsSemiPucca) ||
      formData.dwellingUnitsSemiPucca < 0
    ) {
      newErrors.push({
        field: "dwellingUnitsSemiPucca",
        message: "Dwelling Units - Semi-Pucca is required",
      });
    }
    if (
      formData.dwellingUnitsKatcha === undefined ||
      formData.dwellingUnitsKatcha === null ||
      isNaN(formData.dwellingUnitsKatcha) ||
      formData.dwellingUnitsKatcha < 0
    ) {
      newErrors.push({
        field: "dwellingUnitsKatcha",
        message: "Dwelling Units - Katcha is required",
      });
    }
    if (
      formData.dwellingUnitsTotal === undefined ||
      formData.dwellingUnitsTotal === null ||
      isNaN(formData.dwellingUnitsTotal) ||
      formData.dwellingUnitsTotal < 0
    ) {
      newErrors.push({
        field: "dwellingUnitsTotal",
        message: "Dwelling Units - Total is required",
      });
    }
    if (
      formData.landTenureWithPatta === undefined ||
      formData.landTenureWithPatta === null ||
      isNaN(formData.landTenureWithPatta) ||
      formData.landTenureWithPatta < 0
    ) {
      newErrors.push({
        field: "landTenureWithPatta",
        message: "Land Tenure With Patta is required",
      });
    }
    if (
      formData.landTenurePossessionCertificate === undefined ||
      formData.landTenurePossessionCertificate === null ||
      isNaN(formData.landTenurePossessionCertificate) ||
      formData.landTenurePossessionCertificate < 0
    ) {
      newErrors.push({
        field: "landTenurePossessionCertificate",
        message: "Land Tenure With Possession Certificate is required",
      });
    }
    if (
      formData.landTenureEncroachedPrivate === undefined ||
      formData.landTenureEncroachedPrivate === null ||
      isNaN(formData.landTenureEncroachedPrivate) ||
      formData.landTenureEncroachedPrivate < 0
    ) {
      newErrors.push({
        field: "landTenureEncroachedPrivate",
        message: "Land Tenure Encroached on Private Property is required",
      });
    }
    if (
      formData.landTenureEncroachedPublic === undefined ||
      formData.landTenureEncroachedPublic === null ||
      isNaN(formData.landTenureEncroachedPublic) ||
      formData.landTenureEncroachedPublic < 0
    ) {
      newErrors.push({
        field: "landTenureEncroachedPublic",
        message: "Land Tenure Encroached on Public Property is required",
      });
    }
    if (
      formData.landTenureOnRent === undefined ||
      formData.landTenureOnRent === null ||
      isNaN(formData.landTenureOnRent) ||
      formData.landTenureOnRent < 0
    ) {
      newErrors.push({
        field: "landTenureOnRent",
        message: "Land Tenure On Rent is required",
      });
    }
    ...
    if (!formData.waterSupplyPipelinesExisting) {
      newErrors.push({
        field: "waterSupplyPipelinesExisting",
        message: "Water Supply Pipelines - Existing is required",
      });
    }
    if (!formData.waterSupplyPipelinesAdditional) {
      newErrors.push({
        field: "waterSupplyPipelinesAdditional",
        message: "Water Supply Pipelines - Additional Requirement is required",
      });
    }
    if (
      (formData.waterSupplyPipelinesAdditional || 0) > 0 &&
      (formData.waterSupplyPipelinesCost === undefined ||
        formData.waterSupplyPipelinesCost === null ||
        isNaN(formData.waterSupplyPipelinesCost) ||
        (formData.waterSupplyPipelinesCost || 0) < 0)
    ) {
      newErrors.push({
        field: "waterSupplyPipelinesCost",
        message:
          "Water Supply Pipelines - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.waterSupplyIndividualTapsExisting) {
      newErrors.push({
        field: "waterSupplyIndividualTapsExisting",
        message: "Water Supply Individual Taps - Existing is required",
      });
    }
    if (!formData.waterSupplyIndividualTapsAdditional) {
      newErrors.push({
        field: "waterSupplyIndividualTapsAdditional",
        message:
          "Water Supply Individual Taps - Additional Requirement is required",
      });
    }
    if (
      (formData.waterSupplyIndividualTapsAdditional || 0) > 0 &&
      (formData.waterSupplyIndividualTapsCost === undefined ||
        formData.waterSupplyIndividualTapsCost === null ||
        isNaN(formData.waterSupplyIndividualTapsCost) ||
        (formData.waterSupplyIndividualTapsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "waterSupplyIndividualTapsCost",
        message:
          "Water Supply Individual Taps - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.waterSupplyBorewellsExisting) {
      newErrors.push({
        field: "waterSupplyBorewellsExisting",
        message: "Water Supply Borewells - Existing is required",
      });
    }
    if (!formData.waterSupplyBorewellsAdditional) {
      newErrors.push({
        field: "waterSupplyBorewellsAdditional",
        message: "Water Supply Borewells - Additional Requirement is required",
      });
    }
    if (
      (formData.waterSupplyBorewellsAdditional || 0) > 0 &&
      (formData.waterSupplyBorewellsCost === undefined ||
        formData.waterSupplyBorewellsCost === null ||
        isNaN(formData.waterSupplyBorewellsCost) ||
        (formData.waterSupplyBorewellsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "waterSupplyBorewellsCost",
        message:
          "Water Supply Borewells - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.waterSupplyConnectivityTrunkLinesExisting) {
      newErrors.push({
        field: "waterSupplyConnectivityTrunkLinesExisting",
        message:
          "Water Supply Connectivity to Trunk Lines - Existing is required",
      });
    }
    if (!formData.waterSupplyConnectivityTrunkLinesAdditional) {
      newErrors.push({
        field: "waterSupplyConnectivityTrunkLinesAdditional",
        message:
          "Water Supply Connectivity to Trunk Lines - Additional Requirement is required",
      });
    }
    if (
      (formData.waterSupplyConnectivityTrunkLinesAdditional || 0) > 0 &&
      (formData.waterSupplyConnectivityTrunkLinesCost === undefined ||
        formData.waterSupplyConnectivityTrunkLinesCost === null ||
        isNaN(formData.waterSupplyConnectivityTrunkLinesCost) ||
        (formData.waterSupplyConnectivityTrunkLinesCost || 0) < 0)
    ) {
      newErrors.push({
        field: "waterSupplyConnectivityTrunkLinesCost",
        message:
          "Water Supply Connectivity to Trunk Lines - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    ...
    if (!formData.drainageStormwaterDrainageExisting) {
      newErrors.push({
        field: "drainageStormwaterDrainageExisting",
        message: "Drainage Stormwater Drainage - Existing is required",
      });
    }
    if (!formData.drainageStormwaterDrainageAdditional) {
      newErrors.push({
        field: "drainageStormwaterDrainageAdditional",
        message:
          "Drainage Stormwater Drainage - Additional Requirement is required",
      });
    }
    if (
      (formData.drainageStormwaterDrainageAdditional || 0) > 0 &&
      (formData.drainageStormwaterDrainageCost === undefined ||
        formData.drainageStormwaterDrainageCost === null ||
        isNaN(formData.drainageStormwaterDrainageCost) ||
        (formData.drainageStormwaterDrainageCost || 0) < 0)
    ) {
      newErrors.push({
        field: "drainageStormwaterDrainageCost",
        message:
          "Drainage Stormwater Drainage - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.drainageConnectivityMainDrainsExisting) {
      newErrors.push({
        field: "drainageConnectivityMainDrainsExisting",
        message: "Drainage Connectivity to Main Drains - Existing is required",
      });
    }
    if (!formData.drainageConnectivityMainDrainsAdditional) {
      newErrors.push({
        field: "drainageConnectivityMainDrainsAdditional",
        message:
          "Drainage Connectivity to Main Drains - Additional Requirement is required",
      });
    }
    if (
      (formData.drainageConnectivityMainDrainsAdditional || 0) > 0 &&
      (formData.drainageConnectivityMainDrainsCost === undefined ||
        formData.drainageConnectivityMainDrainsCost === null ||
        isNaN(formData.drainageConnectivityMainDrainsCost) ||
        (formData.drainageConnectivityMainDrainsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "drainageConnectivityMainDrainsCost",
        message:
          "Drainage Connectivity to Main Drains - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.drainageSewerLinesExisting) {
      newErrors.push({
        field: "drainageSewerLinesExisting",
        message: "Drainage Sewer Lines - Existing is required",
      });
    }
    if (!(formData.drainageSewerLinesAdditional || 0)) {
      newErrors.push({
        field: "drainageSewerLinesAdditional",
        message: "Drainage Sewer Lines - Additional Requirement is required",
      });
    }
    if (
      (formData.drainageSewerLinesAdditional || 0) > 0 &&
      (formData.drainageSewerLinesCost === undefined ||
        formData.drainageSewerLinesCost === null ||
        isNaN(formData.drainageSewerLinesCost) ||
        (formData.drainageSewerLinesCost || 0) < 0)
    ) {
      newErrors.push({
        field: "drainageSewerLinesCost",
        message:
          "Drainage Sewer Lines - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.drainageConnectivityTrunkSewersExisting) {
      newErrors.push({
        field: "drainageConnectivityTrunkSewersExisting",
        message: "Drainage Connectivity to Trunk Sewers - Existing is required",
      });
    }
    if (!(formData.drainageConnectivityTrunkSewersAdditional || 0)) {
      newErrors.push({
        field: "drainageConnectivityTrunkSewersAdditional",
        message:
          "Drainage Connectivity to Trunk Sewers - Additional Requirement is required",
      });
    }
    if (
      (formData.drainageConnectivityTrunkSewersAdditional || 0) > 0 &&
      (formData.drainageConnectivityTrunkSewersCost === undefined ||
        formData.drainageConnectivityTrunkSewersCost === null ||
        isNaN(formData.drainageConnectivityTrunkSewersCost) ||
        (formData.drainageConnectivityTrunkSewersCost || 0) < 0)
    ) {
      newErrors.push({
        field: "drainageConnectivityTrunkSewersCost",
        message:
          "Drainage Connectivity to Trunk Sewers - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    ...
    if (!formData.roadsInternalRoadsCCExisting) {
      newErrors.push({
        field: "roadsInternalRoadsCCExisting",
        message: "Roads Internal Roads CC - Existing is required",
      });
    }
    if (!(formData.roadsInternalRoadsCCAdditional || 0)) {
      newErrors.push({
        field: "roadsInternalRoadsCCAdditional",
        message: "Roads Internal Roads CC - Additional Requirement is required",
      });
    }
    if (
      (formData.roadsInternalRoadsCCAdditional || 0) > 0 &&
      (formData.roadsInternalRoadsCCCost === undefined ||
        formData.roadsInternalRoadsCCCost === null ||
        isNaN(formData.roadsInternalRoadsCCCost) ||
        (formData.roadsInternalRoadsCCCost || 0) < 0)
    ) {
      newErrors.push({
        field: "roadsInternalRoadsCCCost",
        message:
          "Roads Internal Roads CC - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.roadsInternalRoadsBTExisting) {
      newErrors.push({
        field: "roadsInternalRoadsBTExisting",
        message: "Roads Internal Roads BT - Existing is required",
      });
    }
    if (!(formData.roadsInternalRoadsBTAdditional || 0)) {
      newErrors.push({
        field: "roadsInternalRoadsBTAdditional",
        message: "Roads Internal Roads BT - Additional Requirement is required",
      });
    }
    if (
      (formData.roadsInternalRoadsBTAdditional || 0) > 0 &&
      (formData.roadsInternalRoadsBTCost === undefined ||
        formData.roadsInternalRoadsBTCost === null ||
        isNaN(formData.roadsInternalRoadsBTCost) ||
        (formData.roadsInternalRoadsBTCost || 0) < 0)
    ) {
      newErrors.push({
        field: "roadsInternalRoadsBTCost",
        message:
          "Roads Internal Roads BT - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.roadsInternalRoadsOthersExisting) {
      newErrors.push({
        field: "roadsInternalRoadsOthersExisting",
        message: "Roads Internal Roads Others - Existing is required",
      });
    }
    if (!(formData.roadsInternalRoadsOthersAdditional || 0)) {
      newErrors.push({
        field: "roadsInternalRoadsOthersAdditional",
        message:
          "Roads Internal Roads Others - Additional Requirement is required",
      });
    }
    if (
      (formData.roadsInternalRoadsOthersAdditional || 0) > 0 &&
      (formData.roadsInternalRoadsOthersCost === undefined ||
        formData.roadsInternalRoadsOthersCost === null ||
        isNaN(formData.roadsInternalRoadsOthersCost) ||
        (formData.roadsInternalRoadsOthersCost || 0) < 0)
    ) {
      newErrors.push({
        field: "roadsInternalRoadsOthersCost",
        message:
          "Roads Internal Roads Others - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.roadsApproachRoadsCCExisting) {
      newErrors.push({
        field: "roadsApproachRoadsCCExisting",
        message: "Roads Approach Roads CC - Existing is required",
      });
    }
    if (!(formData.roadsApproachRoadsCCAdditional || 0)) {
      newErrors.push({
        field: "roadsApproachRoadsCCAdditional",
        message: "Roads Approach Roads CC - Additional Requirement is required",
      });
    }
    if (
      (formData.roadsApproachRoadsCCAdditional || 0) > 0 &&
      (formData.roadsApproachRoadsCCCost === undefined ||
        formData.roadsApproachRoadsCCCost === null ||
        isNaN(formData.roadsApproachRoadsCCCost) ||
        (formData.roadsApproachRoadsCCCost || 0) < 0)
    ) {
      newErrors.push({
        field: "roadsApproachRoadsCCCost",
        message:
          "Roads Approach Roads CC - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.roadsApproachRoadsOthersExisting) {
      newErrors.push({
        field: "roadsApproachRoadsOthersExisting",
        message: "Roads Approach Roads Others - Existing is required",
      });
    }
    if (!(formData.roadsApproachRoadsOthersAdditional || 0)) {
      newErrors.push({
        field: "roadsApproachRoadsOthersAdditional",
        message:
          "Roads Approach Roads Others - Additional Requirement is required",
      });
    }
    if (
      (formData.roadsApproachRoadsOthersAdditional || 0) > 0 &&
      (formData.roadsApproachRoadsOthersCost === undefined ||
        formData.roadsApproachRoadsOthersCost === null ||
        isNaN(formData.roadsApproachRoadsOthersCost) ||
        (formData.roadsApproachRoadsOthersCost || 0) < 0)
    ) {
      newErrors.push({
        field: "roadsApproachRoadsOthersCost",
        message:
          "Roads Approach Roads Others - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    ...
    if (!formData.streetLightingPolesExisting) {
      newErrors.push({
        field: "streetLightingPolesExisting",
        message: "Street Lighting Poles - Existing is required",
      });
    }
    if (!(formData.streetLightingPolesAdditional || 0)) {
      newErrors.push({
        field: "streetLightingPolesAdditional",
        message: "Street Lighting Poles - Additional Requirement is required",
      });
    }
    if (
      (formData.streetLightingPolesAdditional || 0) > 0 &&
      (formData.streetLightingPolesCost === undefined ||
        formData.streetLightingPolesCost === null ||
        isNaN(formData.streetLightingPolesCost) ||
        (formData.streetLightingPolesCost || 0) < 0)
    ) {
      newErrors.push({
        field: "streetLightingPolesCost",
        message:
          "Street Lighting Poles - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.streetLightingLightsExisting) {
      newErrors.push({
        field: "streetLightingLightsExisting",
        message: "Street Lighting Lights - Existing is required",
      });
    }
    if (!(formData.streetLightingLightsAdditional || 0)) {
      newErrors.push({
        field: "streetLightingLightsAdditional",
        message: "Street Lighting Lights - Additional Requirement is required",
      });
    }
    if (
      (formData.streetLightingLightsAdditional || 0) > 0 &&
      (formData.streetLightingLightsCost === undefined ||
        formData.streetLightingLightsCost === null ||
        isNaN(formData.streetLightingLightsCost) ||
        (formData.streetLightingLightsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "streetLightingLightsCost",
        message:
          "Street Lighting Lights - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    ...
    if (!formData.sanitationIndividualToiletsExisting) {
      newErrors.push({
        field: "sanitationIndividualToiletsExisting",
        message: "Sanitation Individual Toilets - Existing is required",
      });
    }
    if (!(formData.sanitationIndividualToiletsAdditional || 0)) {
      newErrors.push({
        field: "sanitationIndividualToiletsAdditional",
        message:
          "Sanitation Individual Toilets - Additional Requirement is required",
      });
    }
    if (
      (formData.sanitationIndividualToiletsAdditional || 0) > 0 &&
      (formData.sanitationIndividualToiletsCost === undefined ||
        formData.sanitationIndividualToiletsCost === null ||
        isNaN(formData.sanitationIndividualToiletsCost) ||
        (formData.sanitationIndividualToiletsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "sanitationIndividualToiletsCost",
        message:
          "Sanitation Individual Toilets - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.sanitationCommunityToiletsExisting) {
      newErrors.push({
        field: "sanitationCommunityToiletsExisting",
        message: "Sanitation Community Toilets - Existing is required",
      });
    }
    if (!formData.sanitationCommunityToiletsAdditional) {
      newErrors.push({
        field: "sanitationCommunityToiletsAdditional",
        message:
          "Sanitation Community Toilets - Additional Requirement is required",
      });
    }
    if (
      (formData.sanitationCommunityToiletsAdditional || 0) > 0 &&
      (formData.sanitationCommunityToiletsCost === undefined ||
        formData.sanitationCommunityToiletsCost === null ||
        isNaN(formData.sanitationCommunityToiletsCost) ||
        (formData.sanitationCommunityToiletsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "sanitationCommunityToiletsCost",
        message:
          "Sanitation Community Toilets - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.sanitationSeatsCommunityToiletsExisting) {
      newErrors.push({
        field: "sanitationSeatsCommunityToiletsExisting",
        message: "Sanitation Seats in Community Toilets - Existing is required",
      });
    }
    if (!formData.sanitationSeatsCommunityToiletsAdditional) {
      newErrors.push({
        field: "sanitationSeatsCommunityToiletsAdditional",
        message:
          "Sanitation Seats in Community Toilets - Additional Requirement is required",
      });
    }
    if (
      (formData.sanitationSeatsCommunityToiletsAdditional || 0) > 0 &&
      (formData.sanitationSeatsCommunityToiletsCost === undefined ||
        formData.sanitationSeatsCommunityToiletsCost === null ||
        isNaN(formData.sanitationSeatsCommunityToiletsCost) ||
        (formData.sanitationSeatsCommunityToiletsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "sanitationSeatsCommunityToiletsCost",
        message:
          "Sanitation Seats in Community Toilets - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.sanitationDumperBinsExisting) {
      newErrors.push({
        field: "sanitationDumperBinsExisting",
        message: "Sanitation Dumper Bins - Existing is required",
      });
    }
    if (!formData.sanitationDumperBinsAdditional) {
      newErrors.push({
        field: "sanitationDumperBinsAdditional",
        message: "Sanitation Dumper Bins - Additional Requirement is required",
      });
    }
    if (
      (formData.sanitationDumperBinsAdditional || 0) > 0 &&
      (formData.sanitationDumperBinsCost === undefined ||
        formData.sanitationDumperBinsCost === null ||
        isNaN(formData.sanitationDumperBinsCost) ||
        (formData.sanitationDumperBinsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "sanitationDumperBinsCost",
        message:
          "Sanitation Dumper Bins - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    ...
    if (!formData.communityHallsExisting) {
      newErrors.push({
        field: "communityHallsExisting",
        message: "Community Halls - Existing is required",
      });
    }
    if (!formData.communityHallsAdditional) {
      newErrors.push({
        field: "communityHallsAdditional",
        message: "Community Halls - Additional Requirement is required",
      });
    }
    if (
      (formData.communityHallsAdditional || 0) > 0 &&
      (formData.communityHallsCost === undefined ||
        formData.communityHallsCost === null ||
        isNaN(formData.communityHallsCost) ||
        (formData.communityHallsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "communityHallsCost",
        message:
          "Community Halls - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.communityLivelihoodCentresExisting) {
      newErrors.push({
        field: "communityLivelihoodCentresExisting",
        message: "Community Livelihood Centres - Existing is required",
      });
    }
    if (!formData.communityLivelihoodCentresAdditional) {
      newErrors.push({
        field: "communityLivelihoodCentresAdditional",
        message:
          "Community Livelihood Centres - Additional Requirement is required",
      });
    }
    if (
      (formData.communityLivelihoodCentresAdditional || 0) > 0 &&
      (formData.communityLivelihoodCentresCost === undefined ||
        formData.communityLivelihoodCentresCost === null ||
        isNaN(formData.communityLivelihoodCentresCost) ||
        (formData.communityLivelihoodCentresCost || 0) < 0)
    ) {
      newErrors.push({
        field: "communityLivelihoodCentresCost",
        message:
          "Community Livelihood Centres - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.communityAnganwadisExisting) {
      newErrors.push({
        field: "communityAnganwadisExisting",
        message: "Community Anganwadis - Existing is required",
      });
    }
    if (!formData.communityAnganwadisAdditional) {
      newErrors.push({
        field: "communityAnganwadisAdditional",
        message: "Community Anganwadis - Additional Requirement is required",
      });
    }
    if (
      (formData.communityAnganwadisAdditional || 0) > 0 &&
      (formData.communityAnganwadisCost === undefined ||
        formData.communityAnganwadisCost === null ||
        isNaN(formData.communityAnganwadisCost) ||
        (formData.communityAnganwadisCost || 0) < 0)
    ) {
      newErrors.push({
        field: "communityAnganwadisCost",
        message:
          "Community Anganwadis - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.communityPrimarySchoolsExisting) {
      newErrors.push({
        field: "communityPrimarySchoolsExisting",
        message: "Community Primary Schools - Existing is required",
      });
    }
    if (!formData.communityPrimarySchoolsAdditional) {
      newErrors.push({
        field: "communityPrimarySchoolsAdditional",
        message:
          "Community Primary Schools - Additional Requirement is required",
      });
    }
    if (
      (formData.communityPrimarySchoolsAdditional || 0) > 0 &&
      (formData.communityPrimarySchoolsCost === undefined ||
        formData.communityPrimarySchoolsCost === null ||
        isNaN(formData.communityPrimarySchoolsCost) ||
        (formData.communityPrimarySchoolsCost || 0) < 0)
    ) {
      newErrors.push({
        field: "communityPrimarySchoolsCost",
        message:
          "Community Primary Schools - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.communityHealthCentresExisting) {
      newErrors.push({
        field: "communityHealthCentresExisting",
        message: "Community Health Centres - Existing is required",
      });
    }
    if (!formData.communityHealthCentresAdditional) {
      newErrors.push({
        field: "communityHealthCentresAdditional",
        message:
          "Community Health Centres - Additional Requirement is required",
      });
    }
    if (
      (formData.communityHealthCentresAdditional || 0) > 0 &&
      (formData.communityHealthCentresCost === undefined ||
        formData.communityHealthCentresCost === null ||
        isNaN(formData.communityHealthCentresCost) ||
        (formData.communityHealthCentresCost || 0) < 0)
    ) {
      newErrors.push({
        field: "communityHealthCentresCost",
        message:
          "Community Health Centres - Estimated Cost is required when Additional Requirement is Yes",
      });
    }

    if (!formData.communityOthersExisting) {
      newErrors.push({
        field: "communityOthersExisting",
        message: "Community Others - Existing is required",
      });
    }
    if (!formData.communityOthersAdditional) {
      newErrors.push({
        field: "communityOthersAdditional",
        message: "Community Others - Additional Requirement is required",
      });
    }
    if (
      (formData.communityOthersAdditional || 0) > 0 &&
      (formData.communityOthersCost === undefined ||
        formData.communityOthersCost === null ||
        isNaN(formData.communityOthersCost) ||
        (formData.communityOthersCost || 0) < 0)
    ) {
      newErrors.push({
        field: "communityOthersCost",
        message:
          "Community Others - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    if (!formData.electricityExisting) {
      newErrors.push({
        field: "electricityExisting",
        message: "Electricity - Existing is required",
      });
    }
    if (!formData.electricityAdditional) {
      newErrors.push({
        field: "electricityAdditional",
        message: "Electricity - Additional Requirement is required",
      });
    }
    if (
      (formData.electricityAdditional || 0) > 0 &&
      (formData.electricityCost === undefined ||
        formData.electricityCost === null ||
        isNaN(formData.electricityCost) ||
        (formData.electricityCost || 0) < 0)
    ) {
      newErrors.push({
        field: "electricityCost",
        message:
          "Electricity - Estimated Cost is required when Additional Requirement is Yes",
      });
    }
    return newErrors;
  };
    ...
    try {
      ...
      const surveyData = {
        ...
        housingStatus: {
          ...
          dwellingUnitsWithElectricityPucca:
            finalFormData.dwellingUnitsWithElectricityPucca || 0,
          dwellingUnitsWithElectricitySemiPucca:
            finalFormData.dwellingUnitsWithElectricitySemiPucca || 0,
          dwellingUnitsWithElectricityKatcha:
            finalFormData.dwellingUnitsWithElectricityKatcha || 0,
          dwellingUnitsWithElectricityTotal:
            finalFormData.dwellingUnitsWithElectricityTotal || 0,
          landTenureWithPatta: finalFormData.landTenureWithPatta || 0,
          landTenurePossessionCertificate:
            finalFormData.landTenurePossessionCertificate || 0,
          landTenureEncroachedPrivate:
            finalFormData.landTenureEncroachedPrivate || 0,
          landTenureEncroachedPublic:
            finalFormData.landTenureEncroachedPublic || 0,
        ...
        physicalInfrastructure: {
          sourceDrinkingWater: {
            individualTap:
              finalFormData.sourceDrinkingWater?.individualTap || 0,
            tubewellBorewellHandpump:
              finalFormData.sourceDrinkingWater?.tubewellBorewellHandpump || 0,
            ...
            riverCanalLakeSpring:
              finalFormData.sourceDrinkingWater?.riverCanalLakeSpring || 0,
            waterTanker: finalFormData.sourceDrinkingWater?.waterTanker || 0,
            others: finalFormData.sourceDrinkingWater?.others || 0,
          },
          connectivityCityWaterSupply:
            finalFormData.connectivityCityWaterSupply || "",
          drainageSewerageFacility:
            finalFormData.drainageSewerageFacility || "",
          connectivityStormWaterDrainage:
            finalFormData.connectivityStormWaterDrainage || "",
          connectivitySewerageSystem:
            finalFormData.connectivitySewerageSystem || "",
          proneToFlooding: finalFormData.proneToFlooding || "",
          latrineFacility: finalFormData.latrineFacility || "",
          frequencyOfGarbageDisposal:
            finalFormData.frequencyOfGarbageDisposal || "",
          arrangementForGarbageDisposal:
            finalFormData.arrangementForGarbageDisposal || "",
          frequencyOfClearanceOfOpenDrains:
            finalFormData.frequencyOfClearanceOfOpenDrains || "",
          approachRoadType: finalFormData.approachRoadType || "",
          distanceToNearestMotorableRoad:
            finalFormData.distanceToNearestMotorableRoad || "",
        ...
        socialDevelopment: {
          ...
          livelihoodProductionCentre:
            finalFormData.livelihoodProductionCentre || 0,
          vocationalTrainingCentre: finalFormData.vocationalTrainingCentre || 0,
          streetChildrenRehabilitationCentre:
            finalFormData.streetChildrenRehabilitationCentre || 0,
        ...
        additionalInfrastructure: {
          ...
          waterSupply: {
            pipelines: {
              existing:
                finalFormData.waterSupplyPipelinesExisting !== undefined
                  ? Number(finalFormData.waterSupplyPipelinesExisting)
                  : 0,
              additionalRequirement:
                finalFormData.waterSupplyPipelinesAdditional !== undefined
                  ? Number(finalFormData.waterSupplyPipelinesAdditional)
                  : 0,
              estimatedCost: finalFormData.waterSupplyPipelinesCost || 0,
            },
            individualTaps: {
              existing:
                finalFormData.waterSupplyIndividualTapsExisting !== undefined
                  ? Number(finalFormData.waterSupplyIndividualTapsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.waterSupplyIndividualTapsAdditional !== undefined
                  ? Number(finalFormData.waterSupplyIndividualTapsAdditional)
                  : 0,
              estimatedCost: finalFormData.waterSupplyIndividualTapsCost || 0,
            },
            borewells: {
              existing:
                finalFormData.waterSupplyBorewellsExisting !== undefined
                  ? Number(finalFormData.waterSupplyBorewellsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.waterSupplyBorewellsAdditional !== undefined
                  ? Number(finalFormData.waterSupplyBorewellsAdditional)
                  : 0,
              estimatedCost: finalFormData.waterSupplyBorewellsCost || 0,
            },
            connectivityToTrunkLines: {
              existing:
                finalFormData.waterSupplyConnectivityTrunkLinesExisting !==
                undefined
                  ? Number(
                      finalFormData.waterSupplyConnectivityTrunkLinesExisting,
                    )
                  : 0,
              additionalRequirement:
                finalFormData.waterSupplyConnectivityTrunkLinesAdditional !==
                undefined
                  ? Number(
                      finalFormData.waterSupplyConnectivityTrunkLinesAdditional,
                    )
                  : 0,
              estimatedCost:
                finalFormData.waterSupplyConnectivityTrunkLinesCost || 0,
            },
          },
          // Drainage/Sewerage
          drainageSewerage: {
            stormwaterDrainage: {
              existing:
                finalFormData.drainageStormwaterDrainageExisting !== undefined
                  ? Number(finalFormData.drainageStormwaterDrainageExisting)
                  : 0,
              additionalRequirement:
                finalFormData.drainageStormwaterDrainageAdditional !== undefined
                  ? Number(finalFormData.drainageStormwaterDrainageAdditional)
                  : 0,
              estimatedCost: finalFormData.drainageStormwaterDrainageCost || 0,
            },
            connectivityToMainDrains: {
              existing:
                finalFormData.drainageConnectivityMainDrainsExisting !==
                undefined
                  ? Number(finalFormData.drainageConnectivityMainDrainsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.drainageConnectivityMainDrainsAdditional !==
                undefined
                  ? Number(
                      finalFormData.drainageConnectivityMainDrainsAdditional,
                    )
                  : 0,
              estimatedCost:
                finalFormData.drainageConnectivityMainDrainsCost || 0,
            },
            sewerLines: {
              existing:
                finalFormData.drainageSewerLinesExisting !== undefined
                  ? Number(finalFormData.drainageSewerLinesExisting)
                  : 0,
              additionalRequirement:
                finalFormData.drainageSewerLinesAdditional !== undefined
                  ? Number(finalFormData.drainageSewerLinesAdditional)
                  : 0,
              estimatedCost: finalFormData.drainageSewerLinesCost || 0,
            },
            connectivityToTrunkSewers: {
              existing:
                finalFormData.drainageConnectivityTrunkSewersExisting !==
                undefined
                  ? Number(
                      finalFormData.drainageConnectivityTrunkSewersExisting,
                    )
                  : 0,
              additionalRequirement:
                finalFormData.drainageConnectivityTrunkSewersAdditional !==
                undefined
                  ? Number(
                      finalFormData.drainageConnectivityTrunkSewersAdditional,
                    )
                  : 0,
              estimatedCost:
                finalFormData.drainageConnectivityTrunkSewersCost || 0,
            },
          },
          // Roads
          roads: {
            internalRoadsCC: {
              existing:
                finalFormData.roadsInternalRoadsCCExisting !== undefined
                  ? Number(finalFormData.roadsInternalRoadsCCExisting)
                  : 0,
              additionalRequirement:
                finalFormData.roadsInternalRoadsCCAdditional !== undefined
                  ? Number(finalFormData.roadsInternalRoadsCCAdditional)
                  : 0,
              estimatedCost: finalFormData.roadsInternalRoadsCCCost || 0,
            },
            internalRoadsBT: {
              existing:
                finalFormData.roadsInternalRoadsBTExisting !== undefined
                  ? Number(finalFormData.roadsInternalRoadsBTExisting)
                  : 0,
              additionalRequirement:
                finalFormData.roadsInternalRoadsBTAdditional !== undefined
                  ? Number(finalFormData.roadsInternalRoadsBTAdditional)
                  : 0,
              estimatedCost: finalFormData.roadsInternalRoadsBTCost || 0,
            },
            internalRoadsOthers: {
              existing:
                finalFormData.roadsInternalRoadsOthersExisting !== undefined
                  ? Number(finalFormData.roadsInternalRoadsOthersExisting)
                  : 0,
              additionalRequirement:
                finalFormData.roadsInternalRoadsOthersAdditional !== undefined
                  ? Number(finalFormData.roadsInternalRoadsOthersAdditional)
                  : 0,
              estimatedCost: finalFormData.roadsInternalRoadsOthersCost || 0,
            },
            approachRoadsCC: {
              existing:
                finalFormData.roadsApproachRoadsCCExisting !== undefined
                  ? Number(finalFormData.roadsApproachRoadsCCExisting)
                  : 0,
              additionalRequirement:
                finalFormData.roadsApproachRoadsCCAdditional !== undefined
                  ? Number(finalFormData.roadsApproachRoadsCCAdditional)
                  : 0,
              estimatedCost: finalFormData.roadsApproachRoadsCCCost || 0,
            },
            approachRoadsOthers: {
              existing:
                finalFormData.roadsApproachRoadsOthersExisting !== undefined
                  ? Number(finalFormData.roadsApproachRoadsOthersExisting)
                  : 0,
              additionalRequirement:
                finalFormData.roadsApproachRoadsOthersAdditional !== undefined
                  ? Number(finalFormData.roadsApproachRoadsOthersAdditional)
                  : 0,
          ...
          streetLighting: {
            poles: {
              existing:
                finalFormData.streetLightingPolesExisting !== undefined
                  ? Number(finalFormData.streetLightingPolesExisting)
                  : 0,
              additionalRequirement:
                finalFormData.streetLightingPolesAdditional !== undefined
                  ? Number(finalFormData.streetLightingPolesAdditional)
                  : 0,
              estimatedCost: finalFormData.streetLightingPolesCost || 0,
            },
            lights: {
              existing:
                finalFormData.streetLightingLightsExisting !== undefined
                  ? Number(finalFormData.streetLightingLightsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.streetLightingLightsAdditional !== undefined
                  ? Number(finalFormData.streetLightingLightsAdditional)
                  : 0,
          ...
          sanitation: {
            individualToilets: {
              existing:
                finalFormData.sanitationIndividualToiletsExisting !== undefined
                  ? Number(finalFormData.sanitationIndividualToiletsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.sanitationIndividualToiletsAdditional !==
                undefined
                  ? Number(finalFormData.sanitationIndividualToiletsAdditional)
                  : 0,
              estimatedCost: finalFormData.sanitationIndividualToiletsCost || 0,
            },
            communityToilets: {
              existing:
                finalFormData.sanitationCommunityToiletsExisting !== undefined
                  ? Number(finalFormData.sanitationCommunityToiletsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.sanitationCommunityToiletsAdditional !== undefined
                  ? Number(finalFormData.sanitationCommunityToiletsAdditional)
                  : 0,
              estimatedCost: finalFormData.sanitationCommunityToiletsCost || 0,
            },
            seatsInCommunityToilets: {
              existing:
                finalFormData.sanitationSeatsCommunityToiletsExisting !==
                undefined
                  ? Number(
                      finalFormData.sanitationSeatsCommunityToiletsExisting,
                    )
                  : 0,
              additionalRequirement:
                finalFormData.sanitationSeatsCommunityToiletsAdditional !==
                undefined
                  ? Number(
                      finalFormData.sanitationSeatsCommunityToiletsAdditional,
                    )
                  : 0,
              estimatedCost:
                finalFormData.sanitationSeatsCommunityToiletsCost || 0,
            },
            dumperBins: {
              existing:
                finalFormData.sanitationDumperBinsExisting !== undefined
                  ? Number(finalFormData.sanitationDumperBinsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.sanitationDumperBinsAdditional !== undefined
                  ? Number(finalFormData.sanitationDumperBinsAdditional)
                  : 0,
          ...
          communityFacilities: {
            communityHalls: {
              existing:
                finalFormData.communityHallsExisting !== undefined
                  ? Number(finalFormData.communityHallsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.communityHallsAdditional !== undefined
                  ? Number(finalFormData.communityHallsAdditional)
                  : 0,
              estimatedCost: finalFormData.communityHallsCost || 0,
            },
            livelihoodCentres: {
              existing:
                finalFormData.communityLivelihoodCentresExisting !== undefined
                  ? Number(finalFormData.communityLivelihoodCentresExisting)
                  : 0,
              additionalRequirement:
                finalFormData.communityLivelihoodCentresAdditional !== undefined
                  ? Number(finalFormData.communityLivelihoodCentresAdditional)
                  : 0,
              estimatedCost: finalFormData.communityLivelihoodCentresCost || 0,
            },
            anganwadis: {
              existing:
                finalFormData.communityAnganwadisExisting !== undefined
                  ? Number(finalFormData.communityAnganwadisExisting)
                  : 0,
              additionalRequirement:
                finalFormData.communityAnganwadisAdditional !== undefined
                  ? Number(finalFormData.communityAnganwadisAdditional)
                  : 0,
              estimatedCost: finalFormData.communityAnganwadisCost || 0,
            },
            primarySchools: {
              existing:
                finalFormData.communityPrimarySchoolsExisting !== undefined
                  ? Number(finalFormData.communityPrimarySchoolsExisting)
                  : 0,
              additionalRequirement:
                finalFormData.communityPrimarySchoolsAdditional !== undefined
                  ? Number(finalFormData.communityPrimarySchoolsAdditional)
                  : 0,
              estimatedCost: finalFormData.communityPrimarySchoolsCost || 0,
            },
            healthCentres: {
              existing:
                finalFormData.communityHealthCentresExisting !== undefined
                  ? Number(finalFormData.communityHealthCentresExisting)
                  : 0,
              additionalRequirement:
                finalFormData.communityHealthCentresAdditional !== undefined
                  ? Number(finalFormData.communityHealthCentresAdditional)
                  : 0,
              estimatedCost: finalFormData.communityHealthCentresCost || 0,
            },
            others: {
              existing:
                finalFormData.communityOthersExisting !== undefined
                  ? Number(finalFormData.communityOthersExisting)
                  : 0,
              additionalRequirement:
                finalFormData.communityOthersAdditional !== undefined
                  ? Number(finalFormData.communityOthersAdditional)
                  : 0,
      ...
      const response = await apiService.submitSlumSurvey(
        slumSurvey._id,
        surveyData,
      );
  ...
  const saveSection = async () => {
    try {
      ...
      const extractSectionData = () => {
        ...
        switch (sectionName) {
          ...
          case "housingStatus":
            ...
            data.dwellingUnitsWithElectricityPucca =
              formData.dwellingUnitsWithElectricityPucca || 0;
            data.dwellingUnitsWithElectricitySemiPucca =
              formData.dwellingUnitsWithElectricitySemiPucca || 0;
            data.dwellingUnitsWithElectricityKatcha =
              formData.dwellingUnitsWithElectricityKatcha || 0;
            data.dwellingUnitsWithElectricityTotal =
              formData.dwellingUnitsWithElectricityTotal || 0;
            data.landTenureWithPatta = formData.landTenureWithPatta || 0;
            data.landTenurePossessionCertificate =
              formData.landTenurePossessionCertificate || 0;
            data.landTenureEncroachedPrivate =
              formData.landTenureEncroachedPrivate || 0;
            data.landTenureEncroachedPublic =
              formData.landTenureEncroachedPublic || 0;
          ...
          case "physicalInfrastructure":
            data.sourceDrinkingWater = {
              ...
              tubewellBorewellHandpump:
                formData.sourceDrinkingWater?.tubewellBorewellHandpump || 0,
              ...
              riverCanalLakeSpring:
                formData.sourceDrinkingWater?.riverCanalLakeSpring || 0,
              waterTanker: formData.sourceDrinkingWater?.waterTanker || 0,
              others: formData.sourceDrinkingWater?.others || 0,
            };
            data.connectivityCityWaterSupply =
              formData.connectivityCityWaterSupply || "";
            data.drainageSewerageFacility =
              formData.drainageSewerageFacility || "";
            data.connectivityStormWaterDrainage =
              formData.connectivityStormWaterDrainage || "";
            data.connectivitySewerageSystem =
              formData.connectivitySewerageSystem || "";
            data.proneToFlooding = formData.proneToFlooding || "";
            data.latrineFacility = {
              ownSepticTankFlushLatrine:
                formData.latrineFacility?.ownSepticTankFlushLatrine || 0,
              ownDryLatrine: formData.latrineFacility?.ownDryLatrine || 0,
              sharedSepticTankFlushLatrine:
                formData.latrineFacility?.sharedSepticTankFlushLatrine || 0,
              sharedDryLatrine: formData.latrineFacility?.sharedDryLatrine || 0,
              communitySepticTankFlushLatrine:
                formData.latrineFacility?.communitySepticTankFlushLatrine || 0,
              communityDryLatrine:
                formData.latrineFacility?.communityDryLatrine || 0,
              openDefecation: formData.latrineFacility?.openDefecation || 0,
            };
            data.solidWasteManagement = {
              frequencyOfGarbageDisposal:
                formData.frequencyOfGarbageDisposal || "",
              arrangementForGarbageDisposal:
                formData.arrangementForGarbageDisposal || "",
              frequencyOfClearanceOfOpenDrains:
                formData.frequencyOfClearanceOfOpenDrains || "",
            };
            data.approachRoadType = formData.approachRoadType || "";
            data.distanceToNearestMotorableRoad =
              formData.distanceToNearestMotorableRoad || "";
          ...
          case "socialDevelopment":
            ...
            data.livelihoodProductionCentre =
              formData.livelihoodProductionCentre || 0;
            data.vocationalTrainingCentre =
              formData.vocationalTrainingCentre || 0;
            data.streetChildrenRehabilitationCentre =
              formData.streetChildrenRehabilitationCentre || 0;
            ...
            data.disabledPensionsHolders =
              formData.disabledPensionsHolders || 0;
            data.generalInsuranceCovered =
              formData.generalInsuranceCovered || 0;
            ...
            data.slumDwellersAssociation =
              formData.slumDwellersAssociation || "";
          ...
          case "additionalInfrastructure":
            data.waterSupply = {
              pipelines: {
                existing:
                  formData.waterSupplyPipelinesExisting !== undefined
                    ? Number(formData.waterSupplyPipelinesExisting)
                    : 0,
                additionalRequirement:
                  formData.waterSupplyPipelinesAdditional !== undefined
                    ? Number(formData.waterSupplyPipelinesAdditional)
                    : 0,
                estimatedCost: formData.waterSupplyPipelinesCost || 0,
              },
              individualTaps: {
                existing:
                  formData.waterSupplyIndividualTapsExisting !== undefined
                    ? Number(formData.waterSupplyIndividualTapsExisting)
                    : 0,
                additionalRequirement:
                  formData.waterSupplyIndividualTapsAdditional !== undefined
                    ? Number(formData.waterSupplyIndividualTapsAdditional)
                    : 0,
                estimatedCost: formData.waterSupplyIndividualTapsCost || 0,
              },
              borewells: {
                existing:
                  formData.waterSupplyBorewellsExisting !== undefined
                    ? Number(formData.waterSupplyBorewellsExisting)
                    : 0,
                additionalRequirement:
                  formData.waterSupplyBorewellsAdditional !== undefined
                    ? Number(formData.waterSupplyBorewellsAdditional)
                    : 0,
                estimatedCost: formData.waterSupplyBorewellsCost || 0,
              },
              connectivityToTrunkLines: {
                existing:
                  formData.waterSupplyConnectivityTrunkLinesExisting !==
                  undefined
                    ? Number(formData.waterSupplyConnectivityTrunkLinesExisting)
                    : 0,
                additionalRequirement:
                  formData.waterSupplyConnectivityTrunkLinesAdditional !==
                  undefined
                    ? Number(
                        formData.waterSupplyConnectivityTrunkLinesAdditional,
                      )
                    : 0,
                estimatedCost:
                  formData.waterSupplyConnectivityTrunkLinesCost || 0,
              },
            };
            ...
            data.drainageSewerage = {
              stormwaterDrainage: {
                existing:
                  formData.drainageStormwaterDrainageExisting !== undefined
                    ? Number(formData.drainageStormwaterDrainageExisting)
                    : 0,
                additionalRequirement:
                  formData.drainageStormwaterDrainageAdditional !== undefined
                    ? Number(formData.drainageStormwaterDrainageAdditional)
                    : 0,
                estimatedCost: formData.drainageStormwaterDrainageCost || 0,
              },
              connectivityToMainDrains: {
                existing:
                  formData.drainageConnectivityMainDrainsExisting !== undefined
                    ? Number(formData.drainageConnectivityMainDrainsExisting)
                    : 0,
                additionalRequirement:
                  formData.drainageConnectivityMainDrainsAdditional !==
                  undefined
                    ? Number(formData.drainageConnectivityMainDrainsAdditional)
                    : 0,
                estimatedCost: formData.drainageConnectivityMainDrainsCost || 0,
              },
              sewerLines: {
                existing:
                  formData.drainageSewerLinesExisting !== undefined
                    ? Number(formData.drainageSewerLinesExisting)
                    : 0,
                additionalRequirement:
                  formData.drainageSewerLinesAdditional !== undefined
                    ? Number(formData.drainageSewerLinesAdditional)
                    : 0,
                estimatedCost: formData.drainageSewerLinesCost || 0,
              },
              connectivityToTrunkSewers: {
                existing:
                  formData.drainageConnectivityTrunkSewersExisting !== undefined
                    ? Number(formData.drainageConnectivityTrunkSewersExisting)
                    : 0,
                additionalRequirement:
                  formData.drainageConnectivityTrunkSewersAdditional !==
                  undefined
                    ? Number(formData.drainageConnectivityTrunkSewersAdditional)
                    : 0,
                estimatedCost:
                  formData.drainageConnectivityTrunkSewersCost || 0,
              },
            };
            ...
            data.roads = {
              internalRoadsCC: {
                existing:
                  formData.roadsInternalRoadsCCExisting !== undefined
                    ? Number(formData.roadsInternalRoadsCCExisting)
                    : 0,
                additionalRequirement:
                  formData.roadsInternalRoadsCCAdditional !== undefined
                    ? Number(formData.roadsInternalRoadsCCAdditional)
                    : 0,
                estimatedCost: formData.roadsInternalRoadsCCCost || 0,
              },
              internalRoadsBT: {
                existing:
                  formData.roadsInternalRoadsBTExisting !== undefined
                    ? Number(formData.roadsInternalRoadsBTExisting)
                    : 0,
                additionalRequirement:
                  formData.roadsInternalRoadsBTAdditional !== undefined
                    ? Number(formData.roadsInternalRoadsBTAdditional)
                    : 0,
                estimatedCost: formData.roadsInternalRoadsBTCost || 0,
              },
              internalRoadsOthers: {
                existing:
                  formData.roadsInternalRoadsOthersExisting !== undefined
                    ? Number(formData.roadsInternalRoadsOthersExisting)
                    : 0,
                additionalRequirement:
                  formData.roadsInternalRoadsOthersAdditional !== undefined
                    ? Number(formData.roadsInternalRoadsOthersAdditional)
                    : 0,
                estimatedCost: formData.roadsInternalRoadsOthersCost || 0,
              },
              approachRoadsCC: {
                existing:
                  formData.roadsApproachRoadsCCExisting !== undefined
                    ? Number(formData.roadsApproachRoadsCCExisting)
                    : 0,
                additionalRequirement:
                  formData.roadsApproachRoadsCCAdditional !== undefined
                    ? Number(formData.roadsApproachRoadsCCAdditional)
                    : 0,
                estimatedCost: formData.roadsApproachRoadsCCCost || 0,
              },
              approachRoadsOthers: {
                existing:
                  formData.roadsApproachRoadsOthersExisting !== undefined
                    ? Number(formData.roadsApproachRoadsOthersExisting)
                    : 0,
                additionalRequirement:
                  formData.roadsApproachRoadsOthersAdditional !== undefined
                    ? Number(formData.roadsApproachRoadsOthersAdditional)
                    : 0,
            ...
            data.streetLighting = {
              poles: {
                existing:
                  formData.streetLightingPolesExisting !== undefined
                    ? Number(formData.streetLightingPolesExisting)
                    : 0,
                additionalRequirement:
                  formData.streetLightingPolesAdditional !== undefined
                    ? Number(formData.streetLightingPolesAdditional)
                    : 0,
                estimatedCost: formData.streetLightingPolesCost || 0,
              },
              lights: {
                existing:
                  formData.streetLightingLightsExisting !== undefined
                    ? Number(formData.streetLightingLightsExisting)
                    : 0,
                additionalRequirement:
                  formData.streetLightingLightsAdditional !== undefined
                    ? Number(formData.streetLightingLightsAdditional)
                    : 0,
            ...
            data.sanitation = {
              individualToilets: {
                existing:
                  formData.sanitationIndividualToiletsExisting !== undefined
                    ? Number(formData.sanitationIndividualToiletsExisting)
                    : 0,
                additionalRequirement:
                  formData.sanitationIndividualToiletsAdditional !== undefined
                    ? Number(formData.sanitationIndividualToiletsAdditional)
                    : 0,
                estimatedCost: formData.sanitationIndividualToiletsCost || 0,
              },
              communityToilets: {
                existing:
                  formData.sanitationCommunityToiletsExisting !== undefined
                    ? Number(formData.sanitationCommunityToiletsExisting)
                    : 0,
                additionalRequirement:
                  formData.sanitationCommunityToiletsAdditional !== undefined
                    ? Number(formData.sanitationCommunityToiletsAdditional)
                    : 0,
                estimatedCost: formData.sanitationCommunityToiletsCost || 0,
              },
              seatsInCommunityToilets: {
                existing:
                  formData.sanitationSeatsCommunityToiletsExisting !== undefined
                    ? Number(formData.sanitationSeatsCommunityToiletsExisting)
                    : 0,
                additionalRequirement:
                  formData.sanitationSeatsCommunityToiletsAdditional !==
                  undefined
                    ? Number(formData.sanitationSeatsCommunityToiletsAdditional)
                    : 0,
                estimatedCost:
                  formData.sanitationSeatsCommunityToiletsCost || 0,
              },
              dumperBins: {
                existing:
                  formData.sanitationDumperBinsExisting !== undefined
                    ? Number(formData.sanitationDumperBinsExisting)
                    : 0,
                additionalRequirement:
                  formData.sanitationDumperBinsAdditional !== undefined
                    ? Number(formData.sanitationDumperBinsAdditional)
                    : 0,
            ...
            data.communityFacilities = {
              communityHalls: {
                existing:
                  formData.communityHallsExisting !== undefined
                    ? Number(formData.communityHallsExisting)
                    : 0,
                additionalRequirement:
                  formData.communityHallsAdditional !== undefined
                    ? Number(formData.communityHallsAdditional)
                    : 0,
                estimatedCost: formData.communityHallsCost || 0,
              },
              livelihoodCentres: {
                existing:
                  formData.communityLivelihoodCentresExisting !== undefined
                    ? Number(formData.communityLivelihoodCentresExisting)
                    : 0,
                additionalRequirement:
                  formData.communityLivelihoodCentresAdditional !== undefined
                    ? Number(formData.communityLivelihoodCentresAdditional)
                    : 0,
                estimatedCost: formData.communityLivelihoodCentresCost || 0,
              },
              anganwadis: {
                existing:
                  formData.communityAnganwadisExisting !== undefined
                    ? Number(formData.communityAnganwadisExisting)
                    : 0,
                additionalRequirement:
                  formData.communityAnganwadisAdditional !== undefined
                    ? Number(formData.communityAnganwadisAdditional)
                    : 0,
                estimatedCost: formData.communityAnganwadisCost || 0,
              },
              primarySchools: {
                existing:
                  formData.communityPrimarySchoolsExisting !== undefined
                    ? Number(formData.communityPrimarySchoolsExisting)
                    : 0,
                additionalRequirement:
                  formData.communityPrimarySchoolsAdditional !== undefined
                    ? Number(formData.communityPrimarySchoolsAdditional)
                    : 0,
                estimatedCost: formData.communityPrimarySchoolsCost || 0,
              },
              healthCentres: {
                existing:
                  formData.communityHealthCentresExisting !== undefined
                    ? Number(formData.communityHealthCentresExisting)
                    : 0,
                additionalRequirement:
                  formData.communityHealthCentresAdditional !== undefined
                    ? Number(formData.communityHealthCentresAdditional)
                    : 0,
                estimatedCost: formData.communityHealthCentresCost || 0,
              },
              others: {
                existing:
                  formData.communityOthersExisting !== undefined
                    ? Number(formData.communityOthersExisting)
                    : 0,
                additionalRequirement:
                  formData.communityOthersAdditional !== undefined
                    ? Number(formData.communityOthersAdditional)
                    : 0,
          ...
          case "reviewAndSubmit":
            ...
            Object.keys(formData).forEach((key) => {
              if (
                formData[key as keyof SlumSurveyForm] !== undefined &&
      ...
      if (isEditMode) {
        ...
        if (response.success) {
          showToast(
            `Section saved successfully! (${(response.data as any)?.completionPercentage || 0}% complete)`,
            "success",
          );
  ...
  return (
    <SurveyorLayout username={user?.name || user?.username} fullScreen>
      <div className="max-w-4xl mx-auto w-full pb-12">
        {/* Header */}
        <div className="mb-8 bg-linear-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-2xl p-6 shadow-xl">
          <div className="flex items-start justify-between">
            <div>
              <button
                onClick={handleBackToDashboard}
                className="mb-3 text-sm text-blue-400 hover:text-white flex items-center transition-all duration-200 cursor-pointer group"
              >
        ...
        {/* Form Container */}
        <div className="bg-linear-to-br from-slate-800 to-slate-900 border border-slate-700 rounded-2xl p-6 md:p-8 shadow-2xl animate-slide-up">
          {currentStep === 0 && (
            <div className="space-y-6">
              <h2 className="text-2xl font-bold text-white pb-4 flex items-center gap-3">
                <span className="w-1 h-8 bg-blue-500 rounded-full"></span>
                Part A: General Information - City/Town
              </h2>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-slate-800/30 p-6 rounded-xl border border-slate-700/50">
                <Input
                  label="State Code"
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("cityTownCode", e.target.value)
                  }
                  required
                  name="cityTownCode"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-5 bg-slate-800/30 p-6 rounded-xl border border-slate-700/50">
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("slumType", e.target.value)
                  }
                  required
                  name="slumType"
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("slumIdField", e.target.value)
                  }
                  required
                  name="slumIdField"
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-400 mt-1">
                    Auto-calculated from household survey data
                  </p>
                </div>
                <Input
                ...
                <div>
                  ...
                  />
                  <p className="text-xs text-gray-400 mt-1">
                    Auto-calculated from household survey BPL status
                  </p>
                </div>
                <div>
                  <Input
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("surveyorName", e.target.value)
                  }
                  required
                  readOnly
                  className="bg-slate-800/50 cursor-not-allowed opacity-75"
                ...
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("surveyDate", e.target.value)
                  }
                  required
                  name="surveyDate"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Input
                  ...
                  onChange={(e) =>
                    handleInputChange("wardNumber", e.target.value)
                  }
                  required
                  name="wardNumber"
                ...
                <Select
                  ...
                  onChange={(e) =>
                    handleInputChange("locationCoreOrFringe", e.target.value)
                  }
                  required
                ...
                <Select
                  ...
                  onChange={(e) =>
                    handleInputChange("typeAreaSurrounding", e.target.value)
                  }
                  required
                ...
                <Select
                  ...
                  onChange={(e) =>
                    handleInputChange("physicalLocationSlum", e.target.value)
                  }
                  required
                  ...
                  options={[
                    {
                      value: "ALONG_NALLAH",
                      label: "Along Nallah (Major Stormwater Drain) -01",
                    },
                    {
                      value: "ALONG_OTHER_DRAINS",
                      label: "Along Other Drains - 02",
                    },
                    {
                      value: "ALONG_RAILWAY_LINE",
                      label: "Along Railway Line - 03",
                    },
                    {
                      value: "ALONG_MAJOR_TRANSPORT",
                      label: "Along Major Transport Alignment -04",
                    },
                    {
                      value: "ALONG_RIVER_BANK",
                      label: "Along River / Water Body Bank -05",
                    },
                    {
                      value: "ON_RIVER_BED",
                      label: "On River/ Water Body Bed -06",
                    },
                    {
                      value: "OTHERS_HAZARDOUS",
                      label: "Others (Hazardous or Objectionable) - 07",
                    },
                    {
                      value: "OTHERS_NON_HAZARDOUS",
                      label: "Others (Non-Hazardous/Non-objectionable) - 08",
                    },
                  ]}
                />
              </div>
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Select
                  ...
                  onChange={(e) =>
                    handleInputChange("ownershipLandDetail", e.target.value)
                  }
                  required
                  ...
                  options={[
                    {
                      ...
                      label: "Public: Municipal Corporation",
                    },
                    {
                      value: "STATE_GOVERNMENT",
                      label: "Public: State Government",
                    },
          ...
          {currentStep === 5 && (
            <div className="space-y-6">
              ...
              <div className="space-y-6">
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12e. No. of Women-headed Households
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12f. No of Persons older than 65 Years
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12h. No. of Physically Challenged Persons
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12i. No. of Mentally Challenged Persons
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12j. No. of Persons with HIV-AIDS
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12k. No. of Persons with Tuberculosis
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12l. No. of Persons with Respiratory Diseases including
                    Asthma
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    12m. No. of Persons with Other Chronic Diseases
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    13a. Total No. of Illiterate Persons
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    13d. No. of BPL Illiterate Persons
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div>
                  <h3 className="text-lg font-semibold text-white mb-4">
                    13f. No. of Female BPL Illiterate
                  </h3>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    <Input
                      label="SC"
                ...
                <div className="md:col-span-2">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.dwellingUnitsWithElectricitySemiPucca || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "dwellingUnitsWithElectricitySemiPucca",
              ...
              <div className="space-y-6">
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Input
                      ...
                      error={getFieldError(
                        "sourceDrinkingWater.tubewellBorewellHandpump",
                      )}
                    />
                    <Input
                      label="Public Tap"
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.sourceDrinkingWater?.riverCanalLakeSpring || ""
                      }
                      onChange={(e) =>
                        handleNestedInputChange(
                          "sourceDrinkingWater",
                      ...
                      error={getFieldError(
                        "sourceDrinkingWater.riverCanalLakeSpring",
                      )}
                    />
                    <Input
                      label="Water Tanker"
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <Select
                    ...
                    onChange={(e) =>
                      handleInputChange("proneToFlooding", e.target.value)
                    }
                    required
                    options={[
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-2 gap-4">
                    <Input
                      ...
                      required
                      value={
                        formData.latrineFacility?.ownSepticTankFlushLatrine ||
                        ""
                      }
                      onChange={(e) =>
                        handleNestedInputChange(
                      ...
                      error={getFieldError(
                        "latrineFacility.ownSepticTankFlushLatrine",
                      )}
                    />
                    <Input
                    ...
                    <Input
                      ...
                      error={getFieldError(
                        "latrineFacility.sharedSepticTankFlushLatrine",
                      )}
                    />
                    <Input
                    ...
                    <Input
                      ...
                      error={getFieldError(
                        "latrineFacility.communitySepticTankFlushLatrine",
                      )}
                    />
                    <Input
                      ...
                      required
                      value={
                        formData.latrineFacility?.communityDryLatrine || ""
                      }
                      onChange={(e) =>
                        handleNestedInputChange(
                          "latrineFacility",
                      ...
                      error={getFieldError(
                        "latrineFacility.communityDryLatrine",
                      )}
                    />
                    <Input
                  ...
                  <div className="mb-6">
                    ...
                    <Select
                      ...
                      options={[
                        ...
                        {
                          value: "MUNICIPAL CONTRACTOR",
                          label: "Municipal Contractor",
                        },
                        {
                          value: "RESIDENTS THEMSELVES",
                          label: "Residents themselves",
                        },
                ...
                <div>
                  ...
                  <div className="mb-4">
                    ...
                    <Select
                      ...
                      onChange={(e) =>
                        handleInputChange("approachRoadType", e.target.value)
                      }
                      required
                      options={[
                        { value: "MOTORABLE PUCCA", label: "Motorable pucca" },
                        {
                          value: "MOTORABLE KATCHA",
                          label: "Motorable katcha",
                        },
                        {
                          value: "NON-MOTORABLE PUCCA",
                          label: "Non-motorable pucca",
                        },
                        {
                          value: "NON-MOTORABLE KATCHA",
                  ...
                  <div className="mb-4">
                    ...
                    <Select
                      ...
                      options={[
                        ...
                        {
                          value: "MORE THAN 5.0 KMS",
                          label: "More than 5.0 kms",
                        },
                      ]}
                    />
                  </div>
                  ...
                  <div className="mb-4">
                    ...
                    <Select
                      ...
                      required
                      onChange={(e) =>
                        handleInputChange("internalRoadType", e.target.value)
                      }
                      options={[
                        { value: "MOTORABLE PUCCA", label: "Motorable pucca" },
                        {
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div>
                  <Select
                    ...
                    options={[
                      { value: "No", label: "No" },
                      {
                        value: "Within the slum area",
                        label: "Within the slum area",
                      },
                      {
                        value:
                          "Outside the slum area: with distance Less than 0.5 kms",
                        label:
                          "Outside the slum area: with distance Less than 0.5 kms",
                      },
                      {
                        value:
                          "Outside the slum area: with distance 0.5 to 1.0 km",
                        label:
                          "Outside the slum area: with distance 0.5 to 1.0 km",
                      },
                      {
                        value:
                          "Outside the slum area: with distance 1.0 to 2.0 km",
                        label:
                          "Outside the slum area: with distance 1.0 to 2.0 km",
                      },
                      {
                        value:
                          "Outside the slum area: with distance 2.0 to 5.0 km",
                        label:
                          "Outside the slum area: with distance 2.0 to 5.0 km",
                      },
                      {
                        value:
                          "Outside the slum area: with distance more than 5.0 km",
                        label:
                          "Outside the slum area: with distance more than 5.0 km",
                      },
                    ]}
                  />
                  {formData.anganwadiUnderIcds?.option ===
                ...
                <div>
                  ...
                  />
                  {formData.municipalPreschool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.privatePreschool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.municipalPrimarySchool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.stateGovtPrimarySchool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.privatePrimarySchool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.municipalHighSchool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.stateGovtHighSchool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.privateHighSchool?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.adultEducationCentre?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
                ...
                <div>
                  ...
                  />
                  {formData.nonFormalEducationCentre?.option ===
                    "Within the slum area" && (
                    <Input
                      label="Distance within slum area"
                      type="number"
            ...
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                <Select
                  ...
                  required
                  onChange={(e) =>
                    handleInputChange("urbanHealthPost", e.target.value)
                  }
                  options={[
                    { value: "YES", label: "Yes" },
                    { value: "NO", label: "No" },
                ...
                <Select
                  ...
                  required
                  onChange={(e) =>
                    handleInputChange("primaryHealthCentre", e.target.value)
                  }
                  options={[
                    { value: "YES", label: "Yes" },
                    { value: "NO", label: "No" },
                ...
                <Select
                  ...
                  required
                  onChange={(e) =>
                    handleInputChange("governmentHospital", e.target.value)
                  }
                  options={[
                    { value: "YES", label: "Yes" },
                    { value: "NO", label: "No" },
                ...
                <Select
                  ...
                  required
                  onChange={(e) =>
                    handleInputChange("maternityCentre", e.target.value)
                  }
                  options={[
                    { value: "YES", label: "Yes" },
                    { value: "NO", label: "No" },
                ...
                <Select
                  ...
                  required
                  onChange={(e) =>
                    handleInputChange("privateClinic", e.target.value)
                  }
                  options={[
                    { value: "YES", label: "Yes" },
                    { value: "NO", label: "No" },
                ...
                <Select
                  ...
                  required
                  onChange={(e) =>
                    handleInputChange("ayurvedicDoctor", e.target.value)
                  }
                  options={[
                    { value: "YES", label: "Yes" },
                    { value: "NO", label: "No" },
          ...
          {currentStep === 13 && (
            <div className="space-y-6">
              ...
              <div className="space-y-8">
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    />
                    {(formData.waterSupplyIndividualTapsAdditional || 0) >
                      0 && (
                      <Input
                    ...
                    {(formData.waterSupplyConnectivityTrunkLinesAdditional ||
                      ...
                      <Input
                        ...
                        type="number"
                        value={
                          formData.waterSupplyConnectivityTrunkLinesCost || ""
                        }
                        onChange={(e) =>
                          handleInputChange(
                            "waterSupplyConnectivityTrunkLinesCost",
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.drainageStormwaterDrainageAdditional || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "drainageStormwaterDrainageAdditional",
                    ...
                    />
                    {(formData.drainageStormwaterDrainageAdditional || 0) >
                      0 && (
                      <Input
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.drainageConnectivityMainDrainsExisting || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "drainageConnectivityMainDrainsExisting",
                    ...
                    />
                    {(formData.drainageConnectivityMainDrainsAdditional || 0) >
                      0 && (
                      <Input
                        label="Connectivity to Main Drains (Rmts) - Estimated Cost"
                        type="number"
                        value={
                          formData.drainageConnectivityMainDrainsCost || ""
                        }
                        onChange={(e) =>
                          handleInputChange(
                            "drainageConnectivityMainDrainsCost",
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.drainageConnectivityTrunkSewersExisting || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "drainageConnectivityTrunkSewersExisting",
                    ...
                    />
                    {(formData.drainageConnectivityTrunkSewersAdditional || 0) >
                      0 && (
                      <Input
                        label="Connectivity to Trunk Sewers (Rmts) - Estimated Cost"
                        type="number"
                        value={
                          formData.drainageConnectivityTrunkSewersCost || ""
                        }
                        onChange={(e) =>
                          handleInputChange(
                            "drainageConnectivityTrunkSewersCost",
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.sanitationIndividualToiletsAdditional || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "sanitationIndividualToiletsAdditional",
                    ...
                    />
                    {(formData.sanitationIndividualToiletsAdditional || 0) >
                      0 && (
                      <Input
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.sanitationCommunityToiletsAdditional || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "sanitationCommunityToiletsAdditional",
                    ...
                    />
                    {(formData.sanitationCommunityToiletsAdditional || 0) >
                      0 && (
                      <Input
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.sanitationSeatsCommunityToiletsExisting || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "sanitationSeatsCommunityToiletsExisting",
                    ...
                    />
                    {(formData.sanitationSeatsCommunityToiletsAdditional || 0) >
                      0 && (
                      <Input
                        label="Seats in Community Toilets (Nos.) - Estimated Cost"
                        type="number"
                        value={
                          formData.sanitationSeatsCommunityToiletsCost || ""
                        }
                        onChange={(e) =>
                          handleInputChange(
                            "sanitationSeatsCommunityToiletsCost",
                ...
                <div>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    ...
                    <Input
                      ...
                      type="number"
                      value={
                        formData.communityLivelihoodCentresAdditional || ""
                      }
                      onChange={(e) =>
                        handleInputChange(
                          "communityLivelihoodCentresAdditional",
                    ...
                    />
                    {(formData.communityLivelihoodCentresAdditional || 0) >
                      0 && (
                      <Input
              ...
              <div>
                <h3 className="text-lg font-semibold text-white mb-4">
                  43. Standalone Infrastructure Requirements
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  {/* Electricity */}
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("electricityExisting", e.target.value)
                    }
                    required
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("electricityAdditional", e.target.value)
                    }
                    required
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("healthcareExisting", e.target.value)
                    }
                    required
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("healthcareAdditional", e.target.value)
                    }
                    required
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("toiletsExisting", e.target.value)
                    }
                    required
                  ...
                  <Input
                    ...
                    onChange={(e) =>
                      handleInputChange("toiletsAdditional", e.target.value)
                    }
                    required
          ...
          {currentStep === 14 && (
            <div className="space-y-6">
              {/* General Information */}
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  1. General Information - City/Town
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>State Code:</strong>{" "}
                    {renderValue(formData.stateCode)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>State Name:</strong>{" "}
                    {renderValue(formData.stateName)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>District Code:</strong>{" "}
                    {renderValue(formData.districtCode)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>District Name:</strong>{" "}
                    {renderValue(formData.districtName)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>ULB Code:</strong> {renderValue(formData.ulbCode)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>ULB Name:</strong> {renderValue(formData.ulbName)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>City/Town Code:</strong>{" "}
                    {renderValue(formData.cityTownCode)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households in City/Town:</strong>{" "}
                    {renderValue(formData.cityTownNoHouseholds, true)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  2. City/Town Slum Profile
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Slum Type:</strong> {renderValue(formData.slumType)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Slum ID:</strong>{" "}
                    {renderValue(formData.slumIdField)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Slum Name:</strong> {renderValue(formData.slumName)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Area (sq mtrs):</strong>{" "}
                    {renderValue(formData.areaSqMtrs, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Slum Population:</strong>{" "}
                    {renderValue(formData.slumPopulation, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Slum Households:</strong>{" "}
                    {renderValue(formData.noSlumHouseholds, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population:</strong>{" "}
                    {renderValue(formData.bplPopulation, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households:</strong>{" "}
                    {renderValue(formData.bplHouseholds, true)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  3. Particulars of Survey Operation
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Surveyor Name:</strong>{" "}
                    {renderValue(formData.surveyorName)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Survey Date:</strong>{" "}
                    {renderValue(formData.surveyDate)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  4. Basic Information of Slum
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Ward Number:</strong>{" "}
                    {renderValue(formData.wardNumber)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Ward Name:</strong> {renderValue(formData.wardName)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Zone Number:</strong> {renderValue(formData.zone)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Age of Slum (years):</strong>{" "}
                    {renderValue(formData.ageSlumYears, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Location (Core/Fringe):</strong>{" "}
                    {renderValue(formData.locationCoreOrFringe)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Type of Area Surrounding:</strong>{" "}
                    {renderValue(formData.typeAreaSurrounding)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Physical Location of Slum:</strong>{" "}
                    {renderValue(formData.physicalLocationSlum)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Ownership of Land Detail:</strong>{" "}
                    {formData.ownershipLandDetail || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Specify Ownership:</strong>{" "}
                    {formData.ownershipLandSpecify || "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  6. Population & Health Demographics
                </h3>
                <h3 className="text-lg font-medium mb-3 p-2 bg-blue-500 rounded text-black-800">
                  Population & Health
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Population SC:</strong>{" "}
                    {renderValue(formData.totalPopulationSlumSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Population ST:</strong>{" "}
                    {renderValue(formData.totalPopulationSlumST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Population OBC:</strong>{" "}
                    {renderValue(formData.totalPopulationSlumOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Population Others:</strong>{" "}
                    {renderValue(formData.totalPopulationSlumOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Population:</strong>{" "}
                    {renderValue(formData.totalPopulationSlumTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Population Minorities:</strong>{" "}
                    {renderValue(formData.totalPopulationSlumMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population SC:</strong>{" "}
                    {renderValue(formData.bplPopulationSlumSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population ST:</strong>{" "}
                    {renderValue(formData.bplPopulationSlumST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population OBC:</strong>{" "}
                    {renderValue(formData.bplPopulationSlumOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population Others:</strong>{" "}
                    {renderValue(formData.bplPopulationSlumOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population:</strong>{" "}
                    {renderValue(formData.bplPopulationSlumTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>BPL Population Minorities:</strong>{" "}
                    {renderValue(formData.bplPopulationSlumMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households SC:</strong>{" "}
                    {renderValue(formData.noHouseholdsSlumSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households ST:</strong>{" "}
                    {renderValue(formData.noHouseholdsSlumST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households OBC:</strong>{" "}
                    {renderValue(formData.noHouseholdsSlumOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households Others:</strong>{" "}
                    {renderValue(formData.noHouseholdsSlumOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households:</strong>{" "}
                    {renderValue(formData.noHouseholdsSlumTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Households Minorities:</strong>{" "}
                    {renderValue(formData.noHouseholdsSlumMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households SC:</strong>{" "}
                    {renderValue(formData.noBplHouseholdsSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households ST:</strong>{" "}
                    {renderValue(formData.noBplHouseholdsST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households OBC:</strong>{" "}
                    {renderValue(formData.noBplHouseholdsOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households Others:</strong>{" "}
                    {renderValue(formData.noBplHouseholdsOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households Total:</strong>{" "}
                    {renderValue(formData.noBplHouseholdsTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Households Minorities:</strong>{" "}
                    {renderValue(formData.noBplHouseholdsMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Women Headed Households SC:</strong>{" "}
                    {renderValue(formData.noWomenHeadedHouseholdsSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Women Headed Households ST:</strong>{" "}
                    {renderValue(formData.noWomenHeadedHouseholdsST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Women Headed Households OBC:</strong>{" "}
                    {renderValue(formData.noWomenHeadedHouseholdsOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Women Headed Households Others:</strong>{" "}
                    {renderValue(formData.noWomenHeadedHouseholdsOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Women Headed Households Total:</strong>{" "}
                    {renderValue(formData.noWomenHeadedHouseholdsTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Women Headed Households Minorities:</strong>{" "}
                    {renderValue(
                      formData.noWomenHeadedHouseholdsMinorities,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Older 65 SC:</strong>{" "}
                    {renderValue(formData.noPersonsOlder65SC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Older 65 ST:</strong>{" "}
                    {renderValue(formData.noPersonsOlder65ST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Older 65 OBC:</strong>{" "}
                    {renderValue(formData.noPersonsOlder65OBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Older 65 Others:</strong>{" "}
                    {renderValue(formData.noPersonsOlder65Others, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Older 65 Total:</strong>{" "}
                    {renderValue(formData.noPersonsOlder65Total, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Older 65 Minorities:</strong>{" "}
                    {renderValue(formData.noPersonsOlder65Minorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Child Labourers SC:</strong>{" "}
                    {renderValue(formData.noChildLabourersSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Child Labourers ST:</strong>{" "}
                    {renderValue(formData.noChildLabourersST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Child Labourers OBC:</strong>{" "}
                    {renderValue(formData.noChildLabourersOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Child Labourers Others:</strong>{" "}
                    {renderValue(formData.noChildLabourersOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Child Labourers Total:</strong>{" "}
                    {renderValue(formData.noChildLabourersTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Child Labourers Minorities:</strong>{" "}
                    {renderValue(formData.noChildLabourersMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Physically Challenged SC:</strong>{" "}
                    {renderValue(formData.noPhysicallyChallengedSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Physically Challenged ST:</strong>{" "}
                    {renderValue(formData.noPhysicallyChallengedST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Physically Challenged OBC:</strong>{" "}
                    {renderValue(formData.noPhysicallyChallengedOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Physically Challenged Others:</strong>{" "}
                    {renderValue(formData.noPhysicallyChallengedOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Physically Challenged Total:</strong>{" "}
                    {renderValue(formData.noPhysicallyChallengedTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Physically Challenged Minorities:</strong>{" "}
                    {renderValue(
                      formData.noPhysicallyChallengedMinorities,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Mentally Challenged SC:</strong>{" "}
                    {renderValue(formData.noMentallyChallengedSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Mentally Challenged ST:</strong>{" "}
                    {renderValue(formData.noMentallyChallengedST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Mentally Challenged OBC:</strong>{" "}
                    {renderValue(formData.noMentallyChallengedOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Mentally Challenged Others:</strong>{" "}
                    {renderValue(formData.noMentallyChallengedOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Mentally Challenged Total:</strong>{" "}
                    {renderValue(formData.noMentallyChallengedTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Mentally Challenged Minorities:</strong>{" "}
                    {renderValue(formData.noMentallyChallengedMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons HIV/AIDS SC:</strong>{" "}
                    {renderValue(formData.noPersonsHivaidsSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons HIV/AIDS ST:</strong>{" "}
                    {renderValue(formData.noPersonsHivaidsST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons HIV/AIDS OBC:</strong>{" "}
                    {renderValue(formData.noPersonsHivaidsOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons HIV/AIDS Others:</strong>{" "}
                    {renderValue(formData.noPersonsHivaidsOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons HIV/AIDS Total:</strong>{" "}
                    {renderValue(formData.noPersonsHivaidsTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons HIV/AIDS Minorities:</strong>{" "}
                    {renderValue(formData.noPersonsHivaidsMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Tuberculosis SC:</strong>{" "}
                    {renderValue(formData.noPersonsTuberculosisSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Tuberculosis ST:</strong>{" "}
                    {renderValue(formData.noPersonsTuberculosisST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Tuberculosis OBC:</strong>{" "}
                    {renderValue(formData.noPersonsTuberculosisOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Tuberculosis Others:</strong>{" "}
                    {renderValue(formData.noPersonsTuberculosisOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Tuberculosis Total:</strong>{" "}
                    {renderValue(formData.noPersonsTuberculosisTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Tuberculosis Minorities:</strong>{" "}
                    {renderValue(
                      formData.noPersonsTuberculosisMinorities,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Respiratory SC:</strong>{" "}
                    {renderValue(formData.noPersonsRespiratorySC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Respiratory ST:</strong>{" "}
                    {renderValue(formData.noPersonsRespiratoryST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Respiratory OBC:</strong>{" "}
                    {renderValue(formData.noPersonsRespiratoryOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Respiratory Others:</strong>{" "}
                    {renderValue(formData.noPersonsRespiratoryOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Respiratory Total:</strong>{" "}
                    {renderValue(formData.noPersonsRespiratoryTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Respiratory Minorities:</strong>{" "}
                    {renderValue(formData.noPersonsRespiratoryMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Other Chronic SC:</strong>{" "}
                    {renderValue(formData.noPersonsOtherChronicSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Other Chronic ST:</strong>{" "}
                    {renderValue(formData.noPersonsOtherChronicST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Other Chronic OBC:</strong>{" "}
                    {renderValue(formData.noPersonsOtherChronicOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Other Chronic Others:</strong>{" "}
                    {renderValue(formData.noPersonsOtherChronicOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Other Chronic Total:</strong>{" "}
                    {renderValue(formData.noPersonsOtherChronicTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Persons Other Chronic Minorities:</strong>{" "}
                    {renderValue(
                      formData.noPersonsOtherChronicMinorities,
                      true,
                    )}
                  </div>
                </div>

                <h3 className="text-lg font-medium mt-3 mb-3 bg-blue-500 p-2 rounded text-black-800">
                  Literacy & Education
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Illiterate Persons SC:</strong>{" "}
                    {renderValue(formData.totalIlliteratePersonsSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Illiterate Persons ST:</strong>{" "}
                    {renderValue(formData.totalIlliteratePersonsST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Illiterate Persons OBC:</strong>{" "}
                    {renderValue(formData.totalIlliteratePersonsOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Illiterate Persons Others:</strong>{" "}
                    {renderValue(formData.totalIlliteratePersonsOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Illiterate Persons Total:</strong>{" "}
                    {renderValue(formData.totalIlliteratePersonsTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Total Illiterate Persons Minorities:</strong>{" "}
                    {renderValue(
                      formData.totalIlliteratePersonsMinorities,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male Illiterate SC:</strong>{" "}
                    {renderValue(formData.noMaleIlliterateSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male Illiterate ST:</strong>{" "}
                    {renderValue(formData.noMaleIlliterateST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male Illiterate OBC:</strong>{" "}
                    {renderValue(formData.noMaleIlliterateOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male Illiterate Others:</strong>{" "}
                    {renderValue(formData.noMaleIlliterateOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male Illiterate Total:</strong>{" "}
                    {renderValue(formData.noMaleIlliterateTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male Illiterate Minorities:</strong>{" "}
                    {renderValue(formData.noMaleIlliterateMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female Illiterate SC:</strong>{" "}
                    {renderValue(formData.noFemaleIlliterateSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female Illiterate ST:</strong>{" "}
                    {renderValue(formData.noFemaleIlliterateST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female Illiterate OBC:</strong>{" "}
                    {renderValue(formData.noFemaleIlliterateOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female Illiterate Others:</strong>{" "}
                    {renderValue(formData.noFemaleIlliterateOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female Illiterate Total:</strong>{" "}
                    {renderValue(formData.noFemaleIlliterateTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female Illiterate Minorities:</strong>{" "}
                    {renderValue(formData.noFemaleIlliterateMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Illiterate Persons SC:</strong>{" "}
                    {renderValue(formData.noBplIlliteratePersonsSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Illiterate Persons ST:</strong>{" "}
                    {renderValue(formData.noBplIlliteratePersonsST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Illiterate Persons OBC:</strong>{" "}
                    {renderValue(formData.noBplIlliteratePersonsOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Illiterate Persons Others:</strong>{" "}
                    {renderValue(formData.noBplIlliteratePersonsOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Illiterate Persons Total:</strong>{" "}
                    {renderValue(formData.noBplIlliteratePersonsTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of BPL Illiterate Persons Minorities:</strong>{" "}
                    {renderValue(
                      formData.noBplIlliteratePersonsMinorities,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male BPL Illiterate SC:</strong>{" "}
                    {renderValue(formData.noMaleBplIlliterateSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male BPL Illiterate ST:</strong>{" "}
                    {renderValue(formData.noMaleBplIlliterateST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male BPL Illiterate OBC:</strong>{" "}
                    {renderValue(formData.noMaleBplIlliterateOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male BPL Illiterate Others:</strong>{" "}
                    {renderValue(formData.noMaleBplIlliterateOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male BPL Illiterate Total:</strong>{" "}
                    {renderValue(formData.noMaleBplIlliterateTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Male BPL Illiterate Minorities:</strong>{" "}
                    {renderValue(formData.noMaleBplIlliterateMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female BPL Illiterate SC:</strong>{" "}
                    {renderValue(formData.noFemaleBplIlliterateSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female BPL Illiterate ST:</strong>{" "}
                    {renderValue(formData.noFemaleBplIlliterateST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female BPL Illiterate OBC:</strong>{" "}
                    {renderValue(formData.noFemaleBplIlliterateOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female BPL Illiterate Others:</strong>{" "}
                    {renderValue(formData.noFemaleBplIlliterateOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female BPL Illiterate Total:</strong>{" "}
                    {renderValue(formData.noFemaleBplIlliterateTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>No. of Female BPL Illiterate Minorities:</strong>{" "}
                    {renderValue(
                      formData.noFemaleBplIlliterateMinorities,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Male SC:</strong>{" "}
                    {renderValue(formData.schoolDropoutsMaleSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Male ST:</strong>{" "}
                    {renderValue(formData.schoolDropoutsMaleST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Male OBC:</strong>{" "}
                    {renderValue(formData.schoolDropoutsMaleOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Male Others:</strong>{" "}
                    {renderValue(formData.schoolDropoutsMaleOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Male Total:</strong>{" "}
                    {renderValue(formData.schoolDropoutsMaleTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Male Minorities:</strong>{" "}
                    {renderValue(formData.schoolDropoutsMaleMinorities, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Female SC:</strong>{" "}
                    {renderValue(formData.schoolDropoutsFemaleSC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Female ST:</strong>{" "}
                    {renderValue(formData.schoolDropoutsFemaleST, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Female OBC:</strong>{" "}
                    {renderValue(formData.schoolDropoutsFemaleOBC, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Female Others:</strong>{" "}
                    {renderValue(formData.schoolDropoutsFemaleOthers, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Female Total:</strong>{" "}
                    {renderValue(formData.schoolDropoutsFemaleTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>School Dropouts Female Minorities:</strong>{" "}
                    {renderValue(formData.schoolDropoutsFemaleMinorities, true)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units Pucca:</strong>{" "}
                    {renderValue(formData.dwellingUnitsPucca, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units Semi-Pucca:</strong>{" "}
                    {renderValue(formData.dwellingUnitsSemiPucca, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units Katcha:</strong>{" "}
                    {renderValue(formData.dwellingUnitsKatcha, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units Total:</strong>{" "}
                    {renderValue(formData.dwellingUnitsTotal, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units with Electricity Pucca:</strong>{" "}
                    {renderValue(
                      formData.dwellingUnitsWithElectricityPucca,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units with Electricity Semi-Pucca:</strong>{" "}
                    {renderValue(
                      formData.dwellingUnitsWithElectricitySemiPucca,
                      true,
                    )}
                  ...
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units with Electricity Katcha:</strong>{" "}
                    {renderValue(
                      formData.dwellingUnitsWithElectricityKatcha,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Dwelling Units with Electricity Total:</strong>{" "}
                    {renderValue(
                      formData.dwellingUnitsWithElectricityTotal,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure with Patta:</strong>{" "}
                    {renderValue(formData.landTenureWithPatta, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure Possession Certificate:</strong>{" "}
                    {renderValue(
                      formData.landTenurePossessionCertificate,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure Encroached Private:</strong>{" "}
                    {renderValue(formData.landTenureEncroachedPrivate, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure Encroached Public:</strong>{" "}
                    {renderValue(formData.landTenureEncroachedPublic, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure On Rent:</strong>{" "}
                    {renderValue(formData.landTenureOnRent, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure Other:</strong>{" "}
                    {renderValue(formData.landTenureOther, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Land Tenure Total:</strong>{" "}
                    {renderValue(formData.landTenureTotal, true)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  8. Economic Status of Households
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>&lt; Rs 500:</strong>{" "}
                    {renderValue(formData.economicStatus?.lessThan500, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Rs 500-1000:</strong>{" "}
                    {renderValue(formData.economicStatus?.rs500to1000, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Rs 1000-1500:</strong>{" "}
                    {renderValue(formData.economicStatus?.rs1000to1500, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Rs 1500-2000:</strong>{" "}
                    {renderValue(formData.economicStatus?.rs1500to2000, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Rs 2000-3000:</strong>{" "}
                    {renderValue(formData.economicStatus?.rs2000to3000, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>&gt; Rs 3000:</strong>{" "}
                    {renderValue(formData.economicStatus?.moreThan3000, true)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  9. Employment and Occupation Status
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Self Employed:</strong>{" "}
                    {renderValue(
                      formData.occupationalStatus?.selfEmployed,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Salaried:</strong>{" "}
                    {renderValue(formData.occupationalStatus?.salaried, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Regular Wage:</strong>{" "}
                    {renderValue(
                      formData.occupationalStatus?.regularWage,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Casual Labour:</strong>{" "}
                    {renderValue(
                      formData.occupationalStatus?.casualLabour,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Others:</strong>{" "}
                    {renderValue(formData.occupationalStatus?.others, true)}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  10. Access to Physical Infrastructure
                </h3>
                <h3 className="text-lg font-medium mb-3 bg-blue-500 p-2 rounded text-black-800">
                  Source of Drinking Water
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Individual Tap:</strong>{" "}
                    {formData.sourceDrinkingWater?.individualTap || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Tubewell/Borewell/Handpump:</strong>{" "}
                    {formData.sourceDrinkingWater?.tubewellBorewellHandpump ||
                      "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Public Tap:</strong>{" "}
                    {formData.sourceDrinkingWater?.publicTap || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Open Well:</strong>{" "}
                    {formData.sourceDrinkingWater?.openwell || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Tank/Pond:</strong>{" "}
                    {formData.sourceDrinkingWater?.tankPond || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>River/Canal/Lake/Spring:</strong>{" "}
                    {formData.sourceDrinkingWater?.riverCanalLakeSpring ||
                      "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Water Tanker:</strong>{" "}
                    {formData.sourceDrinkingWater?.waterTanker || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Others:</strong>{" "}
                    {formData.sourceDrinkingWater?.others || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Connectivity City Water Supply:</strong>{" "}
                    {formData.connectivityCityWaterSupply || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Drainage Sewerage Facility:</strong>{" "}
                    {formData.drainageSewerageFacility || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Connectivity Storm Water Drainage:</strong>{" "}
                    {formData.connectivityStormWaterDrainage || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Connectivity Sewerage System:</strong>{" "}
                    {formData.connectivitySewerageSystem || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Prone to Flooding:</strong>{" "}
                    {formData.proneToFlooding || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>
                      Latrine Facility - Own Septic Tank/Flush Latrine:
                    </strong>{" "}
                    {formData.latrineFacility?.ownSepticTankFlushLatrine ||
                      "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Latrine Facility - Own Dry Latrine:</strong>{" "}
                    {formData.latrineFacility?.ownDryLatrine || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>
                      Latrine Facility - Shared Septic Tank/Flush Latrine:
                    </strong>{" "}
                    {formData.latrineFacility?.sharedSepticTankFlushLatrine ||
                      "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Latrine Facility - Shared Dry Latrine:</strong>{" "}
                    {formData.latrineFacility?.sharedDryLatrine || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>
                      Latrine Facility - Community Septic Tank/Flush Latrine:
                    </strong>{" "}
                    {formData.latrineFacility
                      ?.communitySepticTankFlushLatrine || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Latrine Facility - Community Dry Latrine:</strong>{" "}
                    {formData.latrineFacility?.communityDryLatrine || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Latrine Facility - Open Defecation:</strong>{" "}
                    {formData.latrineFacility?.openDefecation || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Frequency of Garbage Disposal:</strong>{" "}
                    {formData.frequencyOfGarbageDisposal || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Arrangement for Garbage Disposal:</strong>{" "}
                    {formData.arrangementForGarbageDisposal || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Frequency of Clearance of Open Drains:</strong>{" "}
                    {formData.frequencyOfClearanceOfOpenDrains || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Street Light Available:</strong>{" "}
                    {formData.streetLightAvailable || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Approach Road Type:</strong>{" "}
                    {formData.approachRoadType || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Distance to Nearest Motorable Road:</strong>{" "}
                    {formData.distanceToNearestMotorableRoad || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Internal Road Type:</strong>{" "}
                    {formData.internalRoadType || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Street Light Availability:</strong>{" "}
                    {formData.streetLightAvailable || "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Anganwadi under ICDS:</strong>{" "}
                    {formData.anganwadiUnderIcds?.option
                      ? `${formData.anganwadiUnderIcds.option}${formData.anganwadiUnderIcds.option === "Within the slum area" && formData.anganwadiUnderIcds.distance !== null ? ` (${formData.anganwadiUnderIcds.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Municipal Preschool:</strong>{" "}
                    {formData.municipalPreschool?.option
                      ? `${formData.municipalPreschool.option}${formData.municipalPreschool.option === "Within the slum area" && formData.municipalPreschool.distance !== null ? ` (${formData.municipalPreschool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Private Preschool:</strong>{" "}
                    {formData.privatePreschool?.option
                      ? `${formData.privatePreschool.option}${formData.privatePreschool.option === "Within the slum area" && formData.privatePreschool.distance !== null ? ` (${formData.privatePreschool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Municipal Primary School:</strong>{" "}
                    {formData.municipalPrimarySchool?.option
                      ? `${formData.municipalPrimarySchool.option}${formData.municipalPrimarySchool.option === "Within the slum area" && formData.municipalPrimarySchool.distance !== null ? ` (${formData.municipalPrimarySchool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>State Government Primary School:</strong>{" "}
                    {formData.stateGovtPrimarySchool?.option
                      ? `${formData.stateGovtPrimarySchool.option}${formData.stateGovtPrimarySchool.option === "Within the slum area" && formData.stateGovtPrimarySchool.distance !== null ? ` (${formData.stateGovtPrimarySchool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Private Primary School:</strong>{" "}
                    {formData.privatePrimarySchool?.option
                      ? `${formData.privatePrimarySchool.option}${formData.privatePrimarySchool.option === "Within the slum area" && formData.privatePrimarySchool.distance !== null ? ` (${formData.privatePrimarySchool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Municipal High School:</strong>{" "}
                    {formData.municipalHighSchool?.option
                      ? `${formData.municipalHighSchool.option}${formData.municipalHighSchool.option === "Within the slum area" && formData.municipalHighSchool.distance !== null ? ` (${formData.municipalHighSchool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>State Government High School:</strong>{" "}
                    {formData.stateGovtHighSchool?.option
                      ? `${formData.stateGovtHighSchool.option}${formData.stateGovtHighSchool.option === "Within the slum area" && formData.stateGovtHighSchool.distance !== null ? ` (${formData.stateGovtHighSchool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Private High School:</strong>{" "}
                    {formData.privateHighSchool?.option
                      ? `${formData.privateHighSchool.option}${formData.privateHighSchool.option === "Within the slum area" && formData.privateHighSchool.distance !== null ? ` (${formData.privateHighSchool.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Adult Education Centre:</strong>{" "}
                    {formData.adultEducationCentre?.option
                      ? `${formData.adultEducationCentre.option}${formData.adultEducationCentre.option === "Within the slum area" && formData.adultEducationCentre.distance !== null ? ` (${formData.adultEducationCentre.distance})` : ""}`
                      : "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Non-formal Education Centre:</strong>{" "}
                    {formData.nonFormalEducationCentre?.option
                      ? `${formData.nonFormalEducationCentre.option}${formData.nonFormalEducationCentre.option === "Within the slum area" && formData.nonFormalEducationCentre.distance !== null ? ` (${formData.nonFormalEducationCentre.distance})` : ""}`
                      : "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Urban Health Post:</strong>{" "}
                    {formData.urbanHealthPost || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Primary Health Centre:</strong>{" "}
                    {formData.primaryHealthCentre || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Government Hospital:</strong>{" "}
                    {formData.governmentHospital || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Maternity Centre:</strong>{" "}
                    {formData.maternityCentre || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Private Clinic:</strong>{" "}
                    {formData.privateClinic || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>RMP:</strong> {formData.rmp || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Ayurvedic Doctor:</strong>{" "}
                    {formData.ayurvedicDoctor || "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="p-2 bg-slate-800 rounded">
                    ...
                    {formData.communityHall || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Livelihood/Production Centre:</strong>{" "}
                    {formData.livelihoodProductionCentre || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Vocational Training Centre:</strong>{" "}
                    {formData.vocationalTrainingCentre || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Street Children Rehabilitation Centre:</strong>{" "}
                    {formData.streetChildrenRehabilitationCentre || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Night Shelter:</strong>{" "}
                    {formData.nightShelter || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Old Age Home:</strong>{" "}
                    {formData.oldAgeHome || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Old Age Pension Holders:</strong>{" "}
                    {formData.oldAgePensionsHolders || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Widow Pension Holders:</strong>{" "}
                    {formData.widowPensionsHolders || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Disabled Pension Holders:</strong>{" "}
                    {formData.disabledPensionsHolders || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>General Insurance Covered:</strong>{" "}
                    {formData.generalInsuranceCovered || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Health Insurance Covered:</strong>{" "}
                    {formData.healthInsuranceCovered || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Self Help Groups:</strong>{" "}
                    {formData.selfHelpGroups || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Thrift & Credit Societies:</strong>{" "}
                    {formData.thriftCreditSocieties || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Slum Dwellers Association:</strong>{" "}
                    {formData.slumDwellersAssociation || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Youth Associations:</strong>{" "}
                    {formData.youthAssociations || "N/A"}
                  </div>
                  <div className="p-2 bg-slate-800 rounded">
                    <strong>Women&apos;s Associations:</strong>{" "}
                    {formData.womensAssociations || "N/A"}
                  </div>
                </div>
              </div>
              ...
              <div className="mb-6">
                <h3 className="text-lg font-medium mb-3 bg-gray-500 p-2 rounded text-black-800">
                  14. Additional Infrastructure Requirements
                </h3>
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Water Supply */}
                  <div className="p-2 bg-blue-500 rounded col-span-2">
                    <strong>Water Supply</strong>
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Pipelines Existing:</strong>{" "}
                    {renderValue(formData.waterSupplyPipelinesExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Pipelines Additional:</strong>{" "}
                    {renderValue(formData.waterSupplyPipelinesAdditional, true)}
                  </div>
                  {renderValue(formData.waterSupplyPipelinesAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Pipelines Estimated Cost:</strong>{" "}
                      {renderValue(formData.waterSupplyPipelinesCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Individual Taps Existing:</strong>{" "}
                    {renderValue(
                      formData.waterSupplyIndividualTapsExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Individual Taps Additional:</strong>{" "}
                    {renderValue(
                      formData.waterSupplyIndividualTapsAdditional,
                      true,
                    )}
                  </div>
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Individual Taps Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.waterSupplyIndividualTapsCost,
                        true,
                      )}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Borewells Existing:</strong>{" "}
                    {renderValue(formData.waterSupplyBorewellsExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Borewells Additional:</strong>{" "}
                    {renderValue(formData.waterSupplyBorewellsAdditional, true)}
                  </div>
                  {renderValue(formData.waterSupplyBorewellsAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Borewells Estimated Cost:</strong>{" "}
                      {renderValue(formData.waterSupplyBorewellsCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Connectivity Trunk Lines Existing:</strong>{" "}
                    {renderValue(
                      formData.waterSupplyConnectivityTrunkLinesExisting,
                      true,
                    )}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Connectivity Trunk Lines Additional:</strong>{" "}
                    {renderValue(
                      formData.waterSupplyConnectivityTrunkLinesAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Connectivity Trunk Lines Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.waterSupplyConnectivityTrunkLinesCost,
                        true,
                      )}
                  ...
                  {/* Drainage/Sewerage */}
                  <div className="p-2 bg-blue-500 rounded col-span-2 mt-4">
                    <strong>Drainage/Sewerage</strong>
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Stormwater Drainage Existing:</strong>{" "}
                    {renderValue(
                      formData.drainageStormwaterDrainageExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Stormwater Drainage Additional:</strong>{" "}
                    {renderValue(
                      formData.drainageStormwaterDrainageAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Stormwater Drainage Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.drainageStormwaterDrainageCost,
                        true,
                      )}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Connectivity Main Drains Existing:</strong>{" "}
                    {renderValue(
                      formData.drainageConnectivityMainDrainsExisting,
                      true,
                    )}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Connectivity Main Drains Additional:</strong>{" "}
                    {renderValue(
                      formData.drainageConnectivityMainDrainsAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Connectivity Main Drains Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.drainageConnectivityMainDrainsCost,
                        true,
                      )}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Sewer Lines Existing:</strong>{" "}
                    {renderValue(formData.drainageSewerLinesExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Sewer Lines Additional:</strong>{" "}
                    {renderValue(formData.drainageSewerLinesAdditional, true)}
                  </div>
                  {renderValue(formData.drainageSewerLinesAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Sewer Lines Estimated Cost:</strong>{" "}
                      {renderValue(formData.drainageSewerLinesCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Connectivity Trunk Sewers Existing:</strong>{" "}
                    {renderValue(
                      formData.drainageConnectivityTrunkSewersExisting,
                      true,
                    )}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Connectivity Trunk Sewers Additional:</strong>{" "}
                    {renderValue(
                      formData.drainageConnectivityTrunkSewersAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Connectivity Trunk Sewers Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.drainageConnectivityTrunkSewersCost,
                        true,
                      )}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Internal Roads CC Existing:</strong>{" "}
                    {renderValue(formData.roadsInternalRoadsCCExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Internal Roads CC Additional:</strong>{" "}
                    {renderValue(formData.roadsInternalRoadsCCAdditional, true)}
                  </div>
                  {renderValue(formData.roadsInternalRoadsCCAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Internal Roads CC Estimated Cost:</strong>{" "}
                      {renderValue(formData.roadsInternalRoadsCCCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Internal Roads BT Existing:</strong>{" "}
                    {renderValue(formData.roadsInternalRoadsBTExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Internal Roads BT Additional:</strong>{" "}
                    {renderValue(formData.roadsInternalRoadsBTAdditional, true)}
                  </div>
                  {renderValue(formData.roadsInternalRoadsBTAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Internal Roads BT Estimated Cost:</strong>{" "}
                      {renderValue(formData.roadsInternalRoadsBTCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Internal Roads Others Existing:</strong>{" "}
                    {renderValue(
                      formData.roadsInternalRoadsOthersExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Internal Roads Others Additional:</strong>{" "}
                    {renderValue(
                      formData.roadsInternalRoadsOthersAdditional,
                      true,
                    )}
                  </div>
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Internal Roads Others Estimated Cost:</strong>{" "}
                      {renderValue(formData.roadsInternalRoadsOthersCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Approach Roads CC Existing:</strong>{" "}
                    {renderValue(formData.roadsApproachRoadsCCExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Approach Roads CC Additional:</strong>{" "}
                    {renderValue(formData.roadsApproachRoadsCCAdditional, true)}
                  </div>
                  {renderValue(formData.roadsApproachRoadsCCAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Approach Roads CC Estimated Cost:</strong>{" "}
                      {renderValue(formData.roadsApproachRoadsCCCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Approach Roads Others Existing:</strong>{" "}
                    {renderValue(
                      formData.roadsApproachRoadsOthersExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Approach Roads Others Additional:</strong>{" "}
                    {renderValue(
                      formData.roadsApproachRoadsOthersAdditional,
                      true,
                    )}
                  </div>
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Approach Roads Others Estimated Cost:</strong>{" "}
                      {renderValue(formData.roadsApproachRoadsOthersCost, true)}
                    </div>
                  ) : null}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Poles Existing:</strong>{" "}
                    {renderValue(formData.streetLightingPolesExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Poles Additional:</strong>{" "}
                    {renderValue(formData.streetLightingPolesAdditional, true)}
                  </div>
                  {renderValue(formData.streetLightingPolesAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Poles Estimated Cost:</strong>{" "}
                      {renderValue(formData.streetLightingPolesCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Lights Existing:</strong>{" "}
                    {renderValue(formData.streetLightingLightsExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Lights Additional:</strong>{" "}
                    {renderValue(formData.streetLightingLightsAdditional, true)}
                  </div>
                  {renderValue(formData.streetLightingLightsAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Lights Estimated Cost:</strong>{" "}
                      {renderValue(formData.streetLightingLightsCost, true)}
                    </div>
                  ) : null}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Individual Toilets Existing:</strong>{" "}
                    {renderValue(
                      formData.sanitationIndividualToiletsExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Individual Toilets Additional:</strong>{" "}
                    {renderValue(
                      formData.sanitationIndividualToiletsAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Individual Toilets Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.sanitationIndividualToiletsCost,
                        true,
                      )}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Community Toilets Existing:</strong>{" "}
                    {renderValue(
                      formData.sanitationCommunityToiletsExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Community Toilets Additional:</strong>{" "}
                    {renderValue(
                      formData.sanitationCommunityToiletsAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Community Toilets Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.sanitationCommunityToiletsCost,
                        true,
                      )}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Seats in Community Toilets Existing:</strong>{" "}
                    {renderValue(
                      formData.sanitationSeatsCommunityToiletsExisting,
                      true,
                    )}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Seats in Community Toilets Additional:</strong>{" "}
                    {renderValue(
                      formData.sanitationSeatsCommunityToiletsAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>
                        Seats in Community Toilets Estimated Cost:
                      </strong>{" "}
                      {renderValue(
                        formData.sanitationSeatsCommunityToiletsCost,
                        true,
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Dumper Bins Existing:</strong>{" "}
                    {renderValue(formData.sanitationDumperBinsExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Dumper Bins Additional:</strong>{" "}
                    {renderValue(formData.sanitationDumperBinsAdditional, true)}
                  </div>
                  {renderValue(formData.sanitationDumperBinsAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Dumper Bins Estimated Cost:</strong>{" "}
                      {renderValue(formData.sanitationDumperBinsCost, true)}
                    </div>
                  ) : null}
                  ...
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Community Halls Existing:</strong>{" "}
                    {renderValue(formData.communityHallsExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Community Halls Additional:</strong>{" "}
                    {renderValue(formData.communityHallsAdditional, true)}
                  </div>
                  {renderValue(formData.communityHallsAdditional, true) > 0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Community Halls Estimated Cost:</strong>{" "}
                      {renderValue(formData.communityHallsCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Livelihood Centres Existing:</strong>{" "}
                    {renderValue(
                      formData.communityLivelihoodCentresExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Livelihood Centres Additional:</strong>{" "}
                    {renderValue(
                      formData.communityLivelihoodCentresAdditional,
                      true,
                    )}
                  ...
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Livelihood Centres Estimated Cost:</strong>{" "}
                      {renderValue(
                        formData.communityLivelihoodCentresCost,
                        true,
                      )}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Anganwadis Existing:</strong>{" "}
                    {renderValue(formData.communityAnganwadisExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Anganwadis Additional:</strong>{" "}
                    {renderValue(formData.communityAnganwadisAdditional, true)}
                  </div>
                  {renderValue(formData.communityAnganwadisAdditional, true) >
                  0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Anganwadis Estimated Cost:</strong>{" "}
                      {renderValue(formData.communityAnganwadisCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Primary Schools Existing:</strong>{" "}
                    {renderValue(
                      formData.communityPrimarySchoolsExisting,
                      true,
                    )}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Primary Schools Additional:</strong>{" "}
                    {renderValue(
                      formData.communityPrimarySchoolsAdditional,
                      true,
                    )}
                  </div>
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Primary Schools Estimated Cost:</strong>{" "}
                      {renderValue(formData.communityPrimarySchoolsCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Health Centres Existing:</strong>{" "}
                    {renderValue(formData.communityHealthCentresExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Health Centres Additional:</strong>{" "}
                    {renderValue(
                      formData.communityHealthCentresAdditional,
                      true,
                    )}
                  </div>
                  {renderValue(
                    ...
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Health Centres Estimated Cost:</strong>{" "}
                      {renderValue(formData.communityHealthCentresCost, true)}
                    </div>
                  ) : null}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Others Existing:</strong>{" "}
                    {renderValue(formData.communityOthersExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Others Additional:</strong>{" "}
                    {renderValue(formData.communityOthersAdditional, true)}
                  </div>
                  {renderValue(formData.communityOthersAdditional, true) > 0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Others Estimated Cost:</strong>{" "}
                      {renderValue(formData.communityOthersCost, true)}
                    </div>
                  ) : null}
                </div>
              </div>
              ...
              <div className="mb-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Electricity */}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Electricity Existing:</strong>{" "}
                    {renderValue(formData.electricityExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Electricity Additional:</strong>{" "}
                    {renderValue(formData.electricityAdditional, true)}
                  </div>
                  {renderValue(formData.electricityAdditional, true) > 0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Electricity Estimated Cost:</strong>{" "}
                      {renderValue(formData.electricityCost, true)}
                    </div>
                  ) : null}

                  {/* Healthcare */}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Healthcare Existing:</strong>{" "}
                    {renderValue(formData.healthcareExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Healthcare Additional:</strong>{" "}
                    {renderValue(formData.healthcareAdditional, true)}
                  </div>
                  {renderValue(formData.healthcareAdditional, true) > 0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Healthcare Estimated Cost:</strong>{" "}
                      {renderValue(formData.healthcareCost, true)}
                    </div>
                  ) : null}

                  {/* Toilets */}
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Toilets Existing:</strong>{" "}
                    {renderValue(formData.toiletsExisting, true)}
                  </div>
                  <div className="p-2 bg-slate-800 rounded col-span-2">
                    <strong>Toilets Additional:</strong>{" "}
                    {renderValue(formData.toiletsAdditional, true)}
                  </div>
                  {renderValue(formData.toiletsAdditional, true) > 0 ? (
                    <div className="p-2 bg-slate-800 rounded col-span-2">
                      <strong>Toilets Estimated Cost:</strong>{" "}
                      {renderValue(formData.toiletsCost, true)}
                    </div>
                  ) : null}
                </div>
              </div>
            ...
            <div className="flex flex-col sm:flex-row gap-3 w-full sm:w-auto">
              {currentStep < steps.length - 1 ? (
                <Button
                  ...
                  onClick={saveSection}
                  disabled={
                    saving || submitting || (isPreviewMode && !isEditMode)
                  }
                ...
                <Button
                  ...
                  onClick={() => {
                    ...
                    console.log(
                      "Button disabled:",
                      submitting || saving || (isPreviewMode && !isEditMode),
                    );
                    handleSubmit();
                  }}
                  disabled={
